...
After each of the two scenarios, participants completed the same nine-item questionnaire. Items~1 to~6 form a TAM-derived short form adapted to the planning domain~\cite{davis_perceived_1989}: items~1 to~3 measure perceived usefulness and items~4 to~6 perceived ease of use, with item~5 reported separately from the ease-of-use mean for the reason given in Section~\ref{sec:eval-questionnaire}. Each is rated on a seven-point Likert scale anchored ``Strongly disagree'' (1) to ``Strongly agree'' (7). Items~7 to~9 capture self-assessed task outcome and are analysed as experience measures rather than as ground-truth completion (Section~\ref{sec:eval-effectiveness}).
...
\begin{table}[htpb]
    ...
    \begin{tabular}{clp{7.6cm}l}
        ...
        \bottomrule
    \end{tabular}
    \caption{The nine-item post-task questionnaire administered after each scenario. \textsuperscript{a}~``Fully successful'', ``Partially successful'', ``Not successful''. \textsuperscript{b}~Anchored ``Very unsatisfied'' (1) to ``Very satisfied'' (7). \textsuperscript{c}~Anchored ``Very difficult'' (1) to ``Very easy'' (7).}
    \label{tab:app-perscenario-instrument}
\end{table}

Item~7 named the concrete constraints for the scenario in question. For Scenario~A the wording was ``e.g.\ semesters 1--7, 30~ECTS $\pm$3, internship semester max 9~ECTS''; for Scenario~B it named the Artificial-Intelligence targets. The questionnaire was presented bilingually (English and German); Table~\ref{tab:app-perscenario-instrument} reproduces the English wording.

\subsection{Final questionnaire (User Experience Questionnaire)}

After both scenarios and the open-exploration phase, participants completed the User Experience Questionnaire once~\cite{laugwitz_construction_2008}. The instrument comprises twenty-six seven-point semantic-differential items between opposing adjectives (for example ``obstructive / supportive'' and ``confusing / clear''), which aggregate into six scales: attractiveness, perspicuity, efficiency, dependability, stimulation, and novelty. Perspicuity, efficiency, and dependability summarise pragmatic (goal-directed) quality; stimulation and novelty summarise hedonic quality. We report the group profile in Table~\ref{tab:ueq}. We use the standard instrument without modification and therefore do not reproduce all twenty-six items here.
...
\begin{longtable}{p{1.6cm}p{9.6cm}cc}
    ...
    E-P13 & ``Disable Graph View'' negative checkbox easy to misread. & sat & 1 \\
    ...
    E-P27 & Internal study wording (``User Study Persona 1'') leaks into the production profile UI. & sat & 1 \\
    ...
    E-P34 & The panel reports ``no missing requirements'' while unmodelled hard constraints (internship semester, lower workload bound) are unmet; the tool green-lights an incomplete plan. & eff & 3 \\
    ...
    E-P41 & New-account creation returns a raw ``username already exists'' error inline, forcing an unguided retry. & eff & 1 \\
    E-P42 & The internships and interdisciplinary recommendation tabs render a bare ``no recommendations'' dead-end. & sat & 1 \\
    ...
    E-P44 & The compliance engine validates against prerequisites and reports them in the dashboard after the fact, but neither view shows them at the point of placement; participants wanted a visible ordering indicator. & eff & 2 \\
    ...
    E-P61 & The ``Estimated Week-Hours per Semester'' panel advertises a target and is never populated. & sat & 1 \\
...
\begin{longtable}{p{1.6cm}p{10.6cm}c}
    ...
    E-G03 & The ``+'' valid-target affordance shows where a course can be placed. & eff \\
    ...
    E-G08 & Graph makes course dependencies and constraints legible. & eff/sat \\
    ...
    E-G11 & The ``Done'' check-off mode with live ECTS and StEOP updates valued. & sat \\
    ...
    E-G16 & Filtering ``Mandatory'' in the graph surfaces all mandatory modules with their affiliations at once, which the table cannot. & eff \\
    E-G17 & The graph clarifies the ``module contains several courses'' relationship where the table's labels confused participants. & sat \\
    E-G18 & Combining a category filter with the ``not planned'' course-state filter turns the graph into a gap-finder. & eff \\
...
Table~\ref{tab:app-completion} gives the final plan state of all twenty-two analysed scenario runs against the constraints of their brief. A lane at or below the stated cap counts as the internship or exchange semester and is exempt from the per-semester band; overshoot of the 180~ECTS total is not counted as a failure, a shortfall is. Lane values were cross-checked against the dashboard's planned total and are reported as unknown where that constraint does not determine them. The excluded session P08 is not listed.
...
\begin{table}[htpb]
    ...
    \begin{tabular}{l p{3.5cm} l l l p{2.5cm}}
        ...
        \midrule
        P01-A & 32/31/32/30/21/31 & none & S5 = 21 & 177 (3 short) & incomplete, 6 of 7 semesters \\
        P01-B & 26/25/20/24/21/22/\textbf{9}/24 & S7 = 9 & S3 = 20 & 171 (9 short) & incomplete \\
        P02-A & 32/31/32/30/27/12/16 & none & S6, S7 & 180 & 2 violations \\
        ...
        P05-A & 26/31/32/12/30/33/16 & none & S1, S4, S7 & 180 & 3 violations \\
        ...
        P12-A & 32/31/?/30/12/30/? & none & S5 = 12, S3/S7 unread & 180 & unresolved \\
        P12-B & 26/25/26/24/21/24/12/22 & none & S7 = 12 & 180 (+6 parked) & 1 violation, no exchange \\
        \bottomrule
    \end{tabular}
    \caption{Final plan state per run against its brief. Scenario~A allows 27 to 33~ECTS per semester over seven semesters, Scenario~B 21 to 27 over eight; both mandate one semester of at most 9~ECTS.}
...
\begin{longtable}{p{5.4cm}cp{6.2cm}}
    ...
    Visible prerequisite and course-ordering indicators & 4/11 & The compliance engine holds the relation and reports it after the fact; neither view renders it at the point of placement (E-P44). \\
