...
Table~\ref{tab:app-matrix} records, for each code, the participants in whom it was evidenced and the resulting frequency. A dot marks presence, an empty cell absence, and a dash a code that could not be decided for that participant, so denominators state how much evidence exists rather than being uniform. A frequency of 3/7 therefore means the code was evidenced in three of the seven participants for whom it could be decided, and was undecidable for the other four; it is not a claim about three of eleven. Codes become undecidable for two reasons. A behaviour may never have been reached in a session, so nothing in the record speaks either way; and the four most recent sessions were captured as screen shares of roughly half the length, at a resolution that does not resolve every lane digit, so codes that depend on reading small numbers or on a later phase of the task cannot be decided for those participants.
