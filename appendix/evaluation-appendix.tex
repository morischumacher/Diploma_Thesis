%% REV (is the ordering good? Maybe start with the scenario briefs, then the post task questionaire, then the UEQ?
...
After each of the two scenarios, participants completed the same nine-item questionnaire. Items~1 to~6 form a TAM-derived short form adapted to the planning %% REV (is this reference correct here?)
domain~\cite{davis_perceived_1989}: items~1 to~3 measure perceived usefulness and items~4 to~6 perceived ease of use, with item~5 reported separately from the ease-of-use mean for the reason given %% REV (match this to the referenced section, is it still discriped there?)
in Section~\ref{sec:eval-questionnaire}. Each is rated on a seven-point Likert scale anchored \enquote{Strongly disagree} (1) to \enquote{Strongly agree} (7). Items~7 to~9 capture self-assessed task outcome and are analysed as experience measures rather than as ground-truth completion (Section~\ref{sec:eval-effectiveness}).
...
\end{table}
%% REV (item 7 named constrains? THis is not corret or? I would delte this here)
...
%% REV (where are the items from this questionaire, i just see one table and its the post task? Also the table is not referenced)
After both scenarios and the open-exploration phase, participants completed the User Experience Questionnaire%% REV (Abrevencion?)
once~\cite{laugwitz_construction_2008}. The instrument comprises twenty-six seven-point semantic-differential items between opposing adjectives (for example \enquote{obstructive / supportive} and \enquote{confusing / clear}), which aggregate into six scales: attractiveness, perspicuity, efficiency, dependability, stimulation, and novelty. Perspicuity, efficiency, and dependability summarise pragmatic (goal-directed) quality; stimulation and novelty summarise hedonic quality. We report the group profile in Table~\ref{tab:ueq}. We use the standard instrument without modification and therefore do not reproduce all twenty-six items here.

\subsection{Voice-interview topics}
\label{app:eval-interview}
%% REV (it this important here, would just delete i think?)
