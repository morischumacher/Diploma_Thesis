...
Both sources of disagreement are systematic, and neither is a misreading. The first is definitional: all eight surface disagreements are the same frame type, the setup modal standing over an empty Table View, which the author coded as the Table View with a modal present and the model coded as the setup surface, exactly as its protocol defines that surface. Reconciling the two definitions by rule brings surface agreement to complete. The second is granularity: every one of the 48 disagreements on the Recommendation Panel, and 35 of the 39 on modals, run in the same direction, and they fall inside stretches the author coded as a block --- the guided onboarding sequence, for instance, was coded once with a note that the panel opens progressively across it, where the model coded each frame for what that frame showed. The model is the more literal coder there, which is what its protocol required.

The two codings differ in one further way, which bears on the plan-state figures rather than on the process states. Over the same 183~frames the author recorded a legible, corroborated lane-ECTS value in 20 and the model in 128. The conservative reading is the author's: where the digits were small in a screen-share capture the author recorded them as unread rather than risk them, whereas the model enlarged the region and transcribed. This is the capability the model was chosen for, and every value it reported was subject to the reconciliation rule above; but it means the per-semester figures rest more heavily on the model's readings than the process states do, and the completion audit of Section~\ref{app:eval-completion} should be read with that in mind.
...
The verdict column distinguishes two kinds of failure, because they are evidence of different things. A \emph{structural} failure misses a constraint on the shape of the plan: the number of semesters, the reduced-load semester the brief mandates, or the degree total. These are the constraints the tool cannot represent at all (Section~\ref{sec:eval-effectiveness}), so nothing on screen could have signalled them. A \emph{band} failure is a lane outside the permitted per-semester range with every other constraint met, and the size of the miss is given, because a lane one credit below the floor and a lane twelve below it are not the same result. The tool displays every lane total and checks none of them against a floor, so a band failure is a number the student could see and the engine never tested.
...
\begin{table}[htpb]
    ...
    \begin{tabular}{@{}l p{3.6cm} l >{\raggedright\arraybackslash}p{1.8cm} l >{\raggedright\arraybackslash}p{2.5cm}@{}}
        ...
        \midrule
        P01-A & 32/31/32/30/21/31 & none & S5 = 21 & 177 (3 short) & structural: 6 semesters, no internship, 3 short \\
        P01-B & 26/25/20/24/21/22/\textbf{9}/24 & S7 = 9 & S3 = 20 & 171 (9 short) & structural: 9 short \\
        P02-A & 32/31/32/30/27/12/16 & none & S6, S7 & 180 & structural: no internship \\
        P02-B & 26/25/26/24/\textbf{9}/24/24/22 & S5 = 9 & none & 180 & \textbf{compliant} \\
        P03-A & 26/31/32/30/\textbf{6}/27/31 & S5 = 6 & S1 = 26 & 183 & band: 1 below floor \\
        P03-B & 32/31/\textbf{8}/30/27/31/12/12 & S3 = 8 & 6 of 7 & 183 & band: six lanes, four above ceiling \\
        P04-A & 32/31/32/30/\textbf{9}/31/15 & S5 = 9 & S7 = 15 & 180 & band: 12 below floor \\
        P04-B & 26/25/26/24/\textbf{9}/24/25/24 & S5 = 9 & none & 183 & \textbf{compliant} \\
        P05-A & 26/31/32/12/30/33/16 & none & S1, S4, S7 & 180 & structural: no internship \\
        P05-B & 26/25/26/24/\textbf{6}/27/24/22 & S5 = 6 & none & 180 & \textbf{compliant} \\
        P06-A & 26/31/29/30/30/\textbf{6}/28 & S6 = 6 & S1 = 26 & 180 & band: 1 below floor \\
        P06-B & 26/25/26/24/24/30/\textbf{9}/22 & S7 = 9 & S6 = 30 & 186 & band: 3 above ceiling \\
        P07-A & 26/28/32/30/\textbf{9}/30/28 & S5 = 9 & S1 = 26 & 183 & band: 1 below floor \\
        P07-B & 26/25/26/24/27/24/\textbf{9}/22 & S7 = 9 & none & 183 & \textbf{compliant} \\
        P09-A & 26/31/26/30/24/12/31 & none & S1, S3, S5, S6 & 180 & structural: no internship \\
        P09-B & 26/25/26/24/\textbf{9}/24/24/22 & S5 = 9 & none & 180 & \textbf{compliant} \\
        P10-A & 32/31/32/30/\textbf{9}/21/28 & S5 = 9 & S6 = 21 & 183 & band: 6 below floor \\
        P10-B & 26/25/26/24/24/24/\textbf{9}/22 & S7 = 9 & none & 180 (+6 parked) & \textbf{compliant} \\
        P11-A & 32/31/26/30/30/\textbf{6}/25 & S6 = 6 & S3, S7 & 180 & band: 1 and 2 below floor \\
        P11-B & 26/25/20/24/\textbf{9}/27/24/25 & S5 = 9 & S3 = 20 & 180 & band: 1 below floor \\
        P12-A & 32/31/?/30/12/30/? & none & S5 = 12, S3/S7 unread & 180 & unresolved: 2 lanes unread \\
        P12-B & 26/25/26/24/21/24/12/22 & none & S7 = 12 & 180 (+6 parked) & structural: no exchange \\
        \bottomrule
    \end{tabular}
    \caption{Final plan state per run against its brief. Scenario~A allows 27 to 33~ECTS per semester over seven semesters, Scenario~B 21 to 27 over eight; both mandate one semester of at most 9~ECTS. \enquote{None} in the reduced-load column is itself a failure of the brief and is carried into the verdict as structural. Band verdicts state how far the offending lanes fall outside the permitted range.}
