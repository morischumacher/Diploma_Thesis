...
\TDrev{Report the agreement between the hand labels and the model labels}{Moritz, 2026-09-06: 300 frames were labelled by hand and the full set by a vision-language model run locally; the two sets overlap on those 300. To be added here, from the two label files: per field (surface, panels, sidebar, lane ECTS), percentage agreement and Cohen's $\kappa$, the number of frames compared, and how disagreements were resolved. The model's name and version go here and into the AI-tools disclosure; Section~\ref{sec:eval-analysis} cites this paragraph for the number.}
