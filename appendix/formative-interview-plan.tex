...
The walkthrough ran in six demonstration steps, three in each view, and the moderator paused after every step so that participants could react to what they had just seen. Table~\ref{tab:walkthrough-steps} lists what was shown at each step and the questions put where a step carried one beyond the free comment the think-aloud invited.

\begin{table}[htbp]
    \centering
    \small
    \setlength{\tabcolsep}{4pt}
    \begin{tabular}{@{}cl>{\raggedright\arraybackslash}p{6.1cm}>{\raggedright\arraybackslash}p{4.3cm}@{}}
        \toprule
        Step & View & Shown & Asked at the pause \\
        \midrule
        1 & Table & The overview; the search field; exam subjects in their distinct colours; module numbers within an exam subject; the semester grid with its per-semester ECTS sum & \\
        \addlinespace
        2 & Table & The sidebar, expanded; the difference between exam-subject, module and course items; the M, C and E markers; what the tool tracks & \\
        \addlinespace
        3 & Table & Drag interactions (prototype frames~2 and~3); how the ECTS sum responds to a change; how a module is moved & \\
        \addlinespace
        4 & Graph & The graph at programme level, including the add-to-plan interaction; the stars; the M, C and E markers; the filter controls; exam-subject, module and course items & Can you think of other ways to connect the graph with the table? \\
        \addlinespace
        5 & Graph & The filter controls in use: exam subjects, only added courses, content similarities, formal prerequisites, show all, collapse all & \\
        \addlinespace
        6 & Graph & Similar courses (prototype frame~2) & \\
        \bottomrule
    \end{tabular}
    \caption{The prototype walkthrough of Phase~C, in the order it was demonstrated. Every step was followed by a pause for the participant's reaction; a blank in the last column means that pause carried no question beyond the free comment the think-aloud invited. The closing discussion of recommendations follows the table.}
