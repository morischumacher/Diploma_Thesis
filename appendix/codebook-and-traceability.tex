...
Each row beneath a header is one excerpt: the interview it came from (I1 to I6), the author's English translation, and the German original, so that every translation can be checked against what was said. Ellipses are the analyst's; a few entries record an analytic observation rather than a spoken line; an excerpt that informed several themes appears under each. \texttt{tools/build\_codebook.py} generates the table from the open-coding document \texttt{First\ Open\ Coding.docx}.
