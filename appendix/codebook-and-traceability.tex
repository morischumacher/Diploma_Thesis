This section presents the complete consolidated codebook produced during Step~2 of the formative study's analysis (Chapter~\ref{chap:formative-study}, Section~\ref{sec:theme-consolidation}): the 64 themes, labelled T1--T64, into which the 435 initial codes were grouped, and under each theme every interview excerpt that informed it. Each theme carries its label, the number of interviews it recurred in, and whether Step~3's scope-based review carried it forward or ruled it out (Section~\ref{sec:scope-refinement}), naming the scope boundary in the second case. The features the carried-forward themes fed into are not repeated here; each feature in Section~\ref{app:feature-specs} names the themes it rests on. \enquote{Carried forward} records that verdict and nothing beyond it: the theme entered the requirements derivation of Chapter~\ref{chap:from-themes-to-requirements}, which is not the same as the requirements built on it having been implemented. Four derived requirements were not, and each is stated in its feature's Delineation (Section~\ref{app:feature-specs}) and taken up in Chapter~\ref{chap:discussion}, Section~\ref{sec:disc-unmet}.

The interviews were conducted in German. Excerpts are given in the author's English translation beside the German original, so that every translation can be checked against what was said; translations are the author's throughout. A few entries in the source document record an analytic observation in English instead of a spoken line, and those appear as written. An excerpt that informed more than one theme appears under each of them. Source: the open-coding document of the formative study, \texttt{First\ Open\ Coding.docx}; the appendix table is generated from it by \texttt{tools/build\_codebook.py}.
