...
This appendix records the 13 features derived in Chapter~\ref{chap:from-themes-to-requirements} in full. Chapter~\ref{chap:from-themes-to-requirements} states the derivation procedure and summarises the result in Table~\ref{tab:feature-summary}; the specifications themselves are reproduced here so that every requirement cited from the Design, Implementation, Evaluation, and Discussion chapters can be read in its own evidential context. Features appear in identifier order, which Table~\ref{tab:feature-summary} reorders by importance score, and each is presented in the same structure: title and identifier, associated themes, description and user evidence, derived requirements, and a delineation where one applies. The themes named under each feature are defined, with the interview excerpts that informed them, in Section~\ref{app:codebook}.
