...
%% REV (only place where we have \emph)
%% REV (and go over this intrdocuing text here and keep it as short as possible, this is huge)
...
%% REV (only place where we have \emph)
%% REad text here and check if still true and correct.
%% REV (decide if this matric is really needed, if yes please rework so its presented in a nicer whay like a matrix, if not cut this section and its references. You can consult me in the chat)
Each row traces one feature from its supporting themes (Section~\ref{app:codebook-table}) through to the Design chapter subsections that cite it (found by scanning every \texttt{FEATURE-XXX} occurrence in \texttt{chapters/design.tex} and recording its enclosing subsection). Requirement counts refer to the numbered requirements list under each feature in Section~\ref{chap:from-themes-to-requirements}. \textbf{A section listed here means the feature is mentioned there, not necessarily that every one of its requirements is individually designed for}; see the note below the table for two features where this distinction matters.
