...
\TDmajor{Sweep every \enquote{dependenc*} occurrence against the glossary}{The glossary now fixes the vocabulary: \emph{containment} is the hierarchy relation and is never called a dependency; \emph{dependency} means an ordering relation between courses, in a \emph{formal} (published, blocking, drawn in the evaluated graph) and a \emph{recommended} (curated, advisory, drawn only after the study) form. Sixty-nine occurrences of \texttt{dependenc*} remain in the prose and each must be checked against that mapping, because the recommended form was not visible during the evaluation study and a sentence that implies it was is false. Counts by file: Related Work~15, codebook appendix~11, Discussion~9, Introduction~8, Formative Study~7, Implementation~7, Design~3, Evaluation~3, evaluation appendix~3, interview plan~1, glossary~2. This sweep is part of each chapter's own pass, not a separate one, and is listed in the definition of done in \texttt{ANWEISUNGEN.md}~\S10.}
