...
\TDmajor{Empty index and acronym list}{\texttt{glossaries} is loaded with
\texttt{[acronym,toc]} and \texttt{main.tex} calls both
\texttt{\textbackslash printindex} and \texttt{\textbackslash printglossaries},
but the thesis contains no \texttt{\textbackslash newacronym}, no
\texttt{\textbackslash gls} and no \texttt{\textbackslash index}, so both render
as empty pages. Recommended: comment both out and keep the Appendix glossary as
the only glossary.}

\TDmajor{Sweep every ``dependenc*'' occurrence}{Once the glossary carries a
Dependency entry naming the three senses used here (containment; formal or hard
prerequisite; recommended or soft ordering), check each of the roughly forty
occurrences resolves to the sense intended. A bare ``dependency'' means the
prerequisite sense after that entry exists.}
