...
Transcripts and recordings were analysed using codebook thematic analysis~\cite{braun_thematic_2022}. Unlike the formative study's inductive approach, this evaluation used a deductive, controlled codebook to classify usability problems (\mbox{E-P}), positive findings (\mbox{E-G}), and neutral or methodological observations (\mbox{E-N}). Each code maps to an ISO~9241-11 dimension~\cite{iso9241_11}, and usability problems were assigned a severity rating on Nielsen's five-point scale~\cite{nielsen1993}. To ensure cohort-wide comparability, any new code identified during later sessions was retroactively evaluated for all prior sessions. The resulting matrix is reported in Section~\ref{app:eval-matrix}.
...
Three things sit beside the planning loop without being stations of it. The Parking Stage is what lets the two views work on one plan; the Recommendation Panel accompanies the loop without driving it; and one input to the checklist that steers the loop, the per-semester ECTS thresholds. Each is reported with what it contributed and what it cost.
...
Six items adapted from the TAM complemented the UEQ's global picture with a per-task measure of perceived usefulness and ease of use. As a reworded, three-per-construct short form, they cannot inherit the reliabilities Davis~\cite{davis_perceived_1989} reports, and the data confirm that the check was needed: across the eleven participants the usefulness items reach $\alpha=0.17$ in Scenario~A and $0.56$ in Scenario~B, whereas the two ease-of-use items retained for that construct reach $0.76$ and $0.72$. Ease of use is therefore reported as a construct and usefulness item by item, so that no claim rests on an average the items do not support. Ratings are high in both conditions and the between-condition differences small (Table~\ref{tab:tam}); given the fixed order, these shifts are read as descriptive, and the comparisons that follow are within-scenario and do not depend on it. The instructive result lies in a single item. Satisfaction with the finished plan is the only rating that falls, from 6.27 to 5.91, and the fall is not diffuse: P03 and P11 drop two points each, P05 and P06 one, two rise, and five do not move. The two largest drops belong to the participants whose Scenario~B plans went furthest wrong, P03-B with six lanes outside the band and P11-B one credit below its floor. This is where self-report and audit meet, and they meet on only one of the two outcome items: on the same sheet, the success item rated seven of the eight failed runs successful (Section~\ref{sec:eval-effectiveness}). Students' judgement of the plan tracked its measured quality; their judgement of their own success did not. Four cases cannot fix the size of the effect, but they suffice to show that the two items measure different things and to identify which the study can rely on, a distinction Chapter~\ref{chap:discussion} builds on.
...
\item Supporting surfaces accompanied the loop without driving it. The Parking Stage acted as the bridge between the two views, used to shortlist courses before commitment by all eleven participants (E-G06, 11/11), though its exclusion from system totals caused silent accounting errors. The Recommendation Panel was heavily occupied but never entered the cycle: it was kept open for long intervals, yet seven of eleven participants set it aside in favour of the checklist and search once they could judge the curriculum themselves (E-N05, 7/11).
