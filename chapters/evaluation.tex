...
Both instruments so far report what participants felt. This section reports what they produced. Every finished plan was audited against the constraints of its brief, run by run, and read against what the participant and the tool said about it; the per-run audit is in Section~\ref{app:eval-completion}. All eleven participants rated their Scenario~A outcome a success, and no Scenario~A run satisfied its brief. What the audit adds to that mismatch is where the plans failed, because the failures land at two boundaries of the tool's own model. Four runs fail structurally, on a constraint the engine cannot represent at all: three plan no internship semester, and one plans six semesters and 177~ECTS. Six fail on the per-semester band alone, three of them by a single credit in the first semester, where the engine holds the number, prints it on the lane header, and never tests it against the brief's floor. One run is unresolved. Self-reported success therefore measures felt rather than actual completion (E-N03, 9/11), but not because participants judged carelessly: they agreed with the instrument in front of them, and the instrument was silent on precisely the constraints they missed.

The brief's constraints divide accordingly. The tool can represent the ECTS total, the curriculum's own requirements and a per-semester ceiling, the last through the profile a student sets; it cannot represent a lower bound on a semester's load or a reduced-load semester for an internship or exchange. Against what it can represent, seventeen of the twenty-one resolved runs are clean, and the four that are not are two runs short of the total (P01) and two with lanes above a ceiling left at its default (P03-B, P06-B), which is the configuration failure of Section~\ref{sec:eval-config}. Against the full brief, six of the twenty-two runs met every stated constraint, nine failed on the per-semester band, six failed structurally, and one is unresolved; the per-run audit, with the kind and size of each failure, is in Section~\ref{app:eval-completion}. The gap between seventeen and six is eleven runs that satisfy everything the engine checks and still miss the task, and every one of them misses on the floor, the reduced-load semester, or both: the two constraints students had to carry themselves, with no confirmation either way (E-P25 and E-P26, both 11/11). All six fully compliant runs are Scenario~B runs, a pattern entirely confounded with order (Section~\ref{sec:eval-rate}).
