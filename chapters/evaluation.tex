...
The graph enters that loop at one point. It replaces the catalogue as the surface on which a named gap becomes candidate courses, taking almost the occupancy and the entry count the catalogue vacates, and it leaves the commit step untouched: the share of placements the tool rejects does not move. The reason is structural rather than a preference, since the graph renders no semester dimension, so the consequence of a placement made there cannot be seen from there.

What the graph makes legible is curriculum structure and almost no sequence. Containment edges are drawn and valued, the two prerequisite pairs the Bachelor curriculum fixes are drawn with them, and one out-of-order advisory fired across the twenty-two runs. No other ordering information existed anywhere in the evaluated tool, in a view or in the engine, so the finding bounds itself to a browsable structural view carrying a near-empty prerequisite overlay.

Recommendations accompanied the loop without becoming part of it. The panel was open for a fifth to a quarter of all sampled frames and ten of eleven participants used it, but it was entered a fraction as often as the catalogue or the graph and held far longer per visit, and seven participants browsed it and returned to the checklist and search to decide.

The tool was received well, and the questionnaires locate its weaknesses rather than contradicting the process account. Every UEQ scale is positive, pragmatic quality (+2.28) leads hedonic (+2.00), and the two weakest scales, perspicuity (+1.82) and novelty (+1.77), match the terminology and learnability problems the recordings show.

Where plans missed their brief, they missed it on what the checklist does not model. Thirteen of the twenty-one resolved runs met every constraint; of the eight that did not, four plan no reduced-load semester at all, which is a requirement the tool has no way to express. All twenty-two runs nonetheless ended with the tool reporting no missing requirements, and seven runs their authors rated a success did not meet their brief. The instrument students trusted as their measure of completion was, for the constraints outside its model, silent rather than wrong.
...
The compliance layer certifies plans it has not checked. Its thresholds are supplied by the user rather than by the task, its lower bound only advises where its upper bound blocks, and two hard constraints of both briefs lie outside its model. All twenty-two runs ended with the tool reporting no missing requirements, including four in which no reduced-load semester was planned at all and two that fell short of the degree total. The instrument students trusted as their measure of completion was, for the constraints it does not model, silent rather than wrong. Where plans missed their brief, they missed it on what the checklist does not model: four of the eight failing runs plan no reduced-load semester, the one constraint of both briefs the tool cannot express.
