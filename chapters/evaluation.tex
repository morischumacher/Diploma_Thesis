...
To operationalise this goal, the study was guided by four questions, carried over from the proposal for this thesis, which planned a task-based evaluation of the hybrid interface and of the recommendations embedded in it. The third is widened here from the recommendations alone to every component the tool places in the workflow, the recommendations among them, because the frame record reports them all on the same footing:
...
\begin{itemize}
    ...
    \item \textbf{Components in the workflow:} Where does each of the tool's components sit in that process, and what does it contribute there? Answered for the surfaces that form the loop (Section~\ref{sec:eval-statemodel}) and for the three that sit beside it, the Parking Stage, the Recommendation Panel and the student-set thresholds (Section~\ref{sec:eval-around}).
...
Scenario~A preceded Scenario~B for every participant. A fixed sequence was chosen over counterbalancing because the object of study is how a graph enters a workflow a student has already formed. By fixing the order, every participant encountered the graph at the same point on their learning curve. While counterbalancing is the standard approach to isolate treatment effects, it would have yielded two different baseline workflows within the cohort (one formed with the graph, one without). A consistent baseline workflow was prioritised instead, a decision grounded in Greenwald's analysis: counterbalancing distributes practice effects but does not remove an interaction between treatment and practice~\cite[p.~316]{greenwald1976within}, and that a repeated-measures design is appropriate where the course of learning is itself the object of study, the sequence then being held constant~\cite[pp.~316--318]{greenwald1976within}.
...
Systematic fixed-interval frame sampling was chosen to give uniform, gapless coverage of interface occupancy and dwell times across the cohort, and because it was tractable within this study: hand-coding every state change across 22 recorded runs would have taken considerably longer than the time available. Because the label scheme was fixed prior to analysis, the process states of Section~\ref{sec:eval-statemodel} are empirically derived from the data rather than selectively interpreted by the analyst.
...
a single state. The Recommendation Panel is deliberately not part of a state. It can stand
...
row in Table~\ref{tab:occupancy} and read in Section~\ref{sec:eval-recs}. That decision
also bounds what its numbers mean: a frame records that the panel was open, not that it was
being read, so a participant who opened it once and never consulted it again is
indistinguishable from one who worked in it (Section~\ref{sec:eval-framemethod}).
