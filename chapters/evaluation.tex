...
While the preceding instruments report perceived outcomes, this section audits actual production. Every finished plan was evaluated against the constraints of its brief (Section~\ref{app:eval-completion}). Of the 21 resolved runs, 13 met every constraint. The eight failures trace directly to systemic blind spots and configuration errors: four missed the reduced-load internship semester, two fell short of the degree total, and two exceeded the maximum workload due to the profile configuration failure detailed in Section~\ref{sec:eval-config}. Crucially, user self-reports failed to capture these errors. Participants rated seven failed runs as successful, meaning self-reported success strictly measures \emph{felt} rather than actual completion (E-N03, 9/11).

Participants consistently over-rated their success because they deferred entirely to the system's completion signals. In all 22 runs, the Dashboard ultimately reported zero missing requirements and a completed focus-area checklist. This determinative \enquote{done} state fired even for plans containing workload band violations and missing internship semesters. Crucially, this reflects a configuration mismatch rather than a software defect: the system is fully capable of tracking these parameters, but only if the user manually inputs them into their profile. Because participants failed to explicitly configure the tool to match their task brief, the system correctly evaluated the plans as compliant against its own default settings. This reliance on the default configuration affected the cohort uniformly, with no consistent correlation to prior curriculum experience.

Participant P02 serves as the primary exemplar of this behavioral dynamic. P02 correctly reserved an empty semester lane for the mandated internship. However, because the system lacks a mechanism to explicitly register internship semesters, the Dashboard remained blind to this requirement and instead flagged a missing ECTS total. Trusting the interface over the external brief, P02 populated the empty lane to clear the warning, actively destroying his own compliant plan. This establishes a consistent behavioral pattern: an unrepresented constraint is not merely left unverified; it actively competes with the constraints the checklist does display, and the interface invariably wins. Chapter~\ref{chap:discussion} explores the design implications of this dynamic.
...
\item Supporting surfaces accompanied the loop without driving it. The Parking Stage acted as the essential bridge between views, universally used to shortlist courses before commitment, though its exclusion from system totals caused silent accounting errors. Similarly, Recommendations were heavily occupied but structurally bypassed: the panel was kept open for long intervals, yet seven participants ultimately abandoned it in favor of deterministic search.

\item The tool was received well, and the questionnaires locate its weaknesses rather than contradicting the process account. Every UEQ scale is positive, pragmatic quality (+2.28) leads hedonic (+2.00), and the two weakest scales, perspicuity (+1.82) and novelty (+1.77), map directly to the system's learnability hurdles. Specifically, obscured requirement terminology and hidden winter/summer availability created the most severe friction during the commit stage.

\item Where plans missed their brief, they missed it on constraints the checklist does not model. Thirteen of the 21 resolved runs met every constraint; of the eight that did not, four plan no reduced-load internship semester at all, which is a requirement the default configuration does not express. All 22 runs nonetheless ended with the tool reporting zero missing requirements, and participants rated seven failed runs as successful. The instrument students trusted as their measure of completion was not merely silent on unmodeled constraints; it actively competed with them. When external instructions conflicted with the system's checklist, users systematically altered their plans to satisfy the interface.
\end{itemize}
