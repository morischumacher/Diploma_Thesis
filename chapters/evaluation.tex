...
Both instruments so far report what participants felt. This section reports what they produced. Every finished plan was audited against the constraints of its brief, run by run, and read against what the participant and the tool said about it; the per-run audit, and the rule by which a run counts as compliant, are in Section~\ref{app:eval-completion}. Thirteen of the twenty-one resolved runs met every constraint of their brief, and the eight that did not concentrate on one thing: four have no reduced-load semester at all, which is the constraint the tool cannot represent. Two fall short of the degree total, and two carry a semester above the ceiling the student had configured, which is the configuration failure of Section~\ref{sec:eval-config}. Self-report does not separate these cases. Seven runs were rated a success by the participant who produced them and missed their brief, and no participant reported a failure that the audit did not also record, so self-reported success measures felt rather than actual completion (E-N03, 9/11) and the observational record rather than the questionnaire carries the claim about what was produced.

The brief's constraints divide into two kinds, and the plans have to be read against each separately. The tool can represent the ECTS total, the curriculum's own requirements and a per-semester ceiling, the last through the profile a student sets; it cannot represent a lower bound on a semester's load or a reduced-load semester for an internship or exchange, because it has no minimum and no semester type. Against the constraints it can represent, seventeen of the twenty-one resolved runs are clean; the four that are not are two runs that ended short of the total (P01) and two with semesters above the ceiling planned against a profile still at its default (P03-B, P06-B), which is the configuration failure of Section~\ref{sec:eval-config}. Under the stricter reading, in which the per-semester range the briefs call permitted is enforced at its lower end as well, no Scenario~A run is compliant and the Scenario~B count falls from seven to six; the per-run table carries every lane needed to apply either reading. The difference between seventeen and thirteen is the constraint the tool cannot hold at all: the reduced-load semester was handled by the students themselves, with mixed results and with no confirmation from the tool either way (E-P25 and E-P26, both 11/11). Compliance is almost the same in the two conditions, six runs against seven, so the completion data carries no condition effect (Section~\ref{sec:eval-rate}).

The tool's completion signal ran ahead of the observed state in the same way the self-reports did, which is why the two agree. All twenty-two runs ended with the Dashboard reporting zero missing requirements and a completed focus-area checklist, including a run with four lanes outside the stated band and no internship semester at all, and two runs that ended with credits still in the Parking Stage. The calibration gap did not track experience: some participants over-rated, one was well calibrated, and an experienced participant under-rated a near-complete plan, so no consistent relationship between experience and calibration holds in this sample.

What the checklist did at the boundary of its own model is the sharpest instance of the mechanism. P02 created an empty semester lane specifically to serve as the mandated internship semester, then filled it to twelve~ECTS because the Dashboard reported that six~ECTS were still missing from the total and said nothing about internships. The requirement he had represented himself was invisible to the mechanism he was following, so following the mechanism removed it. This is the study's central design implication, developed in Chapter~\ref{chap:discussion}: a checklist students treat as their measure of completion must cover every constraint they are held to, because a constraint outside its model is not merely unchecked, it is the one the student will trade away to satisfy what is checked.
