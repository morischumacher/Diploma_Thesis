...
While the UEQ captured the system's global usability footprint, the Technology Acceptance Model (TAM) items administered after each task evaluate perceived utility across the two conditions. Overall acceptance ratings remained consistently high in both scenarios (Table~\ref{tab:tam}), and the differences between conditions are small. While both usefulness and ease of use increased in Scenario~B, the fixed-order design precludes attributing this to the graph, as the shift remains collinear with second-task familiarity. Satisfaction with the finished plan is the one item that falls, from 6.27 to 5.91. The mean movement rests on four participants: P03 and P11 drop two points each, P05 and P06 one, while two rise and five do not move. The two largest drops belong to the participants whose Scenario~B plans went furthest wrong, P03-B with six lanes outside the band and P11-B one credit below its floor, which is consistent with the completion audit of Section~\ref{sec:eval-effectiveness} but rests on four cases and is offered as a reading rather than a result.
