...
A plan is what a student builds and what everything else in the tool reads: the courses filed under each semester, the codes marked done and the codes parked, the notes, hour estimates and grades recorded against individual courses, the note on each semester, the chosen focus area, the workload limits, and what the graph remembers between visits, which is its filters, its collapsed nodes and the positions the student has dragged nodes to. One such plan is held for each degree programme a student has opened, and the account holds the set of them.

The Graph View and the Table View are two projections of that plan. Switching view changes how the plan is drawn, never which plan is described, and persistence, the Dashboard, compliance feedback and recommendations are all derived from the same model.

That model is a reducer: a function from the current plan and a described change to the next plan, holding no framework, no clock and no input or output of its own. Marking a course complete, applying a prebuilt plan, switching programme, choosing a focus area and clearing the plan are transitions of this function, each exercisable by calling it rather than by rendering the interface around it. Placing, moving and parking a course are the exception, and in one direction only. The plan holds the semester, as the key each course is filed under, so nothing reads a card's whereabouts off the screen. What is read off the screen is the change: a drag in the Table View hands the reducer the board's node array, and the reducer works out which lane each card came to rest in rather than being told. So those three are described to the model in the interface's terms, which is the one place the two are reconciled in both directions rather than one; Section~\ref{sec:impl-edit-flow} returns to what it costs. The Graph View is not part of this. A node's position there encodes curriculum depth and subject band, its drag is horizontal only, and what it keeps is a remembered offset rather than a semester; adding a course from the graph names its destination lane outright.

The reducer sits in a domain layer together with the term and lane rules, the lane geometry, the graph filters and the prebuilt-plan builders. That layer depends on no framework and on no other layer, which is what allows the planning rules to be tested without a browser.
