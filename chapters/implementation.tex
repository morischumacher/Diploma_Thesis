...
That model is a reducer: a function from the current plan and a described change to the next plan, holding no framework, no clock and no input or output of its own. Marking a course complete, applying a prebuilt plan, switching programme, choosing a focus area and clearing the plan are transitions of this function, each exercisable by calling it rather than by rendering the interface around it. Placing, moving and parking a course work differently. The canvas owns each card's position, and a card's horizontal position is what records the semester it sits in, so a drag updates the canvas and the reducer is then handed the whole node array to derive the new plan from. Exercising one of those three therefore means building that array rather than naming a course and a semester, which is the one place the model is not free of the interface; Section~\ref{sec:impl-edit-flow} returns to what it costs. The reducer sits in a domain layer together with the term and lane rules, the lane geometry, the graph filters and the prebuilt-plan builders. That layer depends on no framework and on no other layer, which is what allows the planning rules to be tested without a browser.
