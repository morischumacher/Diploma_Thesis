...
The service persists the plan and answers the two questions the client does not answer for itself: whether a plan is compliant, and what to suggest next. It is a FastAPI application in four layers (Figure~\ref{fig:impl-backend-components}), each using only the one beneath it. Request handlers validate input, call one use case and shape the reply; they hold no rules and no SQL. Use cases orchestrate the domain and own the transaction boundary; eight service modules cover authentication and sessions, catalogue retrieval, prerequisite relations, plan loading and saving, profile settings, compliance, recommendation, and the capture of study results, which are written as one file per questionnaire submission (Section~\ref{sec:impl-userstudy}). The domain layer holds the two rule-bearing packages, compliance and recommendation, described in the next two sections. Repositories hold every SQL statement in the service and nothing else.

A use case reaches storage only through a unit of work, an object that hands it every repository bound to one connection. It therefore never acquires a connection and never names a concrete repository, so it can be exercised against a substitute without a database, and the statements that must commit together are stated in the use case rather than left to the order in which repositories run. A use case that cannot do what was asked raises a failure named for the cause, such as a programme locked after setup, a start term already fixed, or a profile saved before its setup; one table maps those names to HTTP status codes and is the only place in the service that knows what a status code is. Authentication resolves a user from a session token carried as a cookie or as a bearer token, so the deployed client and a local instance use the same service unchanged. Persistence is a shared PostgreSQL connection pool with parameterised SQL; no object-relational mapper is used.
...
The Compliance Engine realises Feature~\ref{feat:003} and Feature~\ref{feat:005}. Every rule it applies is taken from the published curricula of the two programmes, the Bachelor programme in Computer Science and the Master programme in Software Engineering\footnote{\url{https://informatics.tuwien.ac.at/bachelor/curriculum-ue033521.pdf}, \url{https://informatics.tuwien.ac.at/master/curriculum-ue066937.pdf}; the versions in force during this work were used.}; none originates elsewhere.

Curriculum content and evaluation logic are separated. Each programme's regulations are a JSON document in \texttt{app/curriculum/}: modules and their credit weights, the course-to-module map, the introductory-phase course sets, focus areas and their aliases, prerequisite pairs, and the thresholds, nine constants for the Bachelor programme and five for the Master. The two documents share no schema; the Master's carries no course-to-module map, focus areas or introductory phase, because its curriculum has none. A checker reads its document once, in its constructor, so a curriculum revision is an edit to the document and not to the code.

There is one rule set per programme, selected by programme code, and the two share only the wire format, the result shape, the entry point and the reading of the credit limits. The Bachelor set enforces the introductory phase (\gterm{steop}{StEOP}); the Master set enforces its prerequisite pairs and warns when an exam subject's electives precede its core; neither rule has a counterpart in the other curriculum. A common base class was not introduced: with so little shared, it would have been smaller than the indirection needed to express it. Whether one emerges with a third programme is left to future work.

Evaluating a plan computes its credit totals per semester and per subject and checks them against the curriculum: each semester against the absolute ceiling and the recommended load, module completeness and exam-subject minima, the programme's prerequisite and sequencing rules, and the total credit target. Findings are returned in two tiers so that the interface guides rather than blocks (Section~\ref{sec:overall-scope}). An error, such as a semester above its ceiling or a hard prerequisite broken, rejects the change that caused it and the client rolls the plan back; a warning, such as a semester above its recommended load or an elective placed before its subject's core, is advisory. The same result carries the statistics the Dashboard renders, so interface and rule engine compute from one state. The recommender queries this engine to vet every suggestion.

\subsection{Recommendation Support}
\label{sec:impl-recommender}

The recommendation component exercises the recommendation workflow of the design (Feature~\ref{feat:007}, Feature~\ref{feat:012}); it does not advance recommender research, and OOS5 places the accuracy of its methods out of scope. It combines existing techniques: content-based matching against stated interests and curated course similarities, typical-sequence relations of the kind sequence-based course recommenders propose~\cite{wong_sequence_2018}, and a what-other-students-did lens modelled on collaborative filtering for elective recommendation~\cite{bhumichitr_recommender_2017}. The behavioural lens runs on a synthetically generated cohort of prior-student plans, not on real interaction data, so it stands in for the workflow a data-driven recommender would support without reproducing its performance.

Six channels implement one protocol: a name and a single method that scores a candidate course and states its evidence. Interest matching weights an explicit topic match above an occurrence in the description and a partial word overlap below both, normalised by the number of interests named. Course similarity infers nothing: it reads curated course-to-course links in the catalogue, each carrying the justification shown as the reason (Figure~\ref{fig:impl-shot-recs}). Sequence reads the orderings the curricula fix between named courses. Follow-on scores a candidate by the share of the cohort who took it after a course this student has completed, and reports nothing below a majority share; peer scores it by the share among the cohort members whose plans most resemble this one, and falls back to overall popularity, flagged as such, when the plan is empty. Internship matches the stated career direction against course skills and descriptions. The engine asks the channels in a fixed order, keeps one record per course, sorts by score, and re-checks each of the top hundred against the Compliance Engine by trial placement, dropping any that would add an error or a warning the plan does not already carry; at most fifteen are returned. Because scores are set per channel, a family with a fixed high score, as curated similarity has, occupies the top of the list.

All six families were live during the sessions of Chapter~\ref{chap:evaluation}. Four answer from what the student supplies or from the synthetic cohort, so a family with nothing to work from produces nothing: a student who named no interests saw no interest matches, which is part of why the empty recommendation tab of E-P42 occurred. The sequence family is bounded by the curricula instead: the Bachelor programme fixes two orderings, the same scarcity the graph's overlay drew, and the reason four of the eleven participants asked for ordering information and did not obtain it (E-P44). What the evaluation examines is how students work with recommendations presented in this way, not how well the matching performs; what a production version with real, consented interaction data would require is noted with the limitations.

\section{Data Model}
\label{sec:impl-data}

The database models a programme as a strict hierarchy (Figure~\ref{fig:impl-data-model}): programme, exam subject, module, course. In both catalogues every course belongs to exactly one module, a property the seeded data holds and the schema does not enforce. A materialised view projects the catalogue per programme into one nested JSON document, which the client fetches in a single request; the service merges a student's term-availability overrides onto it before replying, so the shared data is never written by a user action. Metadata that changes independently of the schema, the content descriptors and the curated course similarities, is a JSONB attributes column on the course row.

Per-user data is the account, one saved plan, and one profile per programme. The plan table is keyed on the user and written as an upsert, so an account holds exactly one plan; the profile table is keyed on user and programme, so a student who switches programme keeps a profile for each. No foreign key joins the per-user tables to the catalogue: they name a programme and a course by code, so a plan survives a reseed of the catalogue, which regenerates every identifier. One plan per account sufficed for the study, but it prevents alternative drafts side by side, a limitation returned to in Chapter~\ref{chap:discussion} (Section~\ref{sec:disc-future}).

The schema is built by migration files applied in lexical order at start-up. Files are named by timestamp rather than by sequence number, so concurrent development does not collide; each applied file is recorded with a checksum, and a file that has changed since it was applied stops the service at start-up; there are no down-migrations.
