...
That model is a reducer: a function from the current plan and a described change to the next plan, holding no framework, no clock and no input or output of its own. Marking a course complete, applying a prebuilt plan, switching programme, choosing a focus area and clearing the plan are transitions of this function, each exercisable by calling it rather than by rendering the interface around it. Placing, moving and parking a course are the exception: the canvas owns each card's position and a card's horizontal position is what records its semester, so those three reach the reducer as a redrawn node array rather than as a course and a destination, which is the one place the model is not free of the interface (Section~\ref{sec:impl-edit-flow}).
...
\caption{The whole tool in the Graph View: the catalogue and its filters on the left, the curriculum in the centre, and the Dashboard on the right. The figures that follow show each surface at a readable size.}
