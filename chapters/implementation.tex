...
%% REV (you say that figure 7.2 is decriped by the subsections which follow bottom up. But the subsection headers do not match the box headers from the figure at all, nether structure wise nor header wise)
...
The Graph View does not take part in this. Its node positions encode curriculum depth and subject band, so dragging there changes a remembered offset and nothing else. Adding a course from the graph does reach the plan through the same array, but its lane is named by the %% REV (glossary? Picker)
picker rather than read from where a card was dropped.
...
%% REV (question, is the table edit included in this explanation and figure too?)
