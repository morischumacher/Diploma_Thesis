...
The client realises the interface designed in Chapter~\ref{chap:design} and is the larger of the two applications. It is organised in three layers, each depending only on the one below it (Figure~\ref{fig:impl-frontend-layers}): a domain layer holding the plan and the rules that change it, the feature modules built on top of it, and an API client through which those modules reach the service. The next three subsections take one layer each, beginning with the domain layer, which the other two are arranged around. Two further subsections cut across all three: what the layers add up to on the screen, and the path a single edit takes through them.

\subsection{The Domain Layer}
\label{sec:impl-plan-model}

A plan is what a student builds and what everything else in the tool reads: the courses filed under each semester, the codes marked done and the codes parked, the notes, hour estimates and grades recorded against individual courses, the note on each semester, the chosen focus area, the workload limits, and what the graph remembers between visits, which is its filters, its collapsed nodes and the positions the student has dragged nodes to. One such plan is held for each degree programme a student has opened, and the account holds the set of them. The Graph View and the Table View are two projections of that plan. Switching view changes how the plan is drawn, never which plan is described, and persistence, the Dashboard, compliance feedback and recommendations are all derived from the same model.

That model is a reducer: a pure function from the current plan and a described change to the next plan, holding no framework, no clock and no input or output of its own. Marking a course complete, applying a prebuilt plan, switching programme, choosing a focus area and clearing the plan are dispatched to it as discrete changes, each exercisable by calling the function rather than by rendering the interface around it.

Placing, moving and parking a course in the Table View are handled differently. The board's node array is built from the plan when a programme is opened, and on every commit the array is written back whole rather than as a single described move. The reason is that one interaction rarely moves one card: collision spacing, module group clearance and the relocation of courses stranded by a change of start term shift others at the same time, and writing the array back captures all of them at once. The consequence is that the canvas is an input to the plan and not only a rendering of it, a trade-off examined in Section~\ref{sec:impl-edit-flow}.
...
The layer holds more than the plan and its reducer. The rules for terms and lanes, which decide the season a lane carries and whether a course may be offered in it; the lane geometry that turns a drop position into a semester; the graph's filter matching, which keeps a matched course's whole ancestry on screen; and the builders behind the prebuilt plans all live there on the same terms, each callable on its own without a browser around it.
...
\label{sec:impl-api-client}

Everything the client asks of the service passes through a single module. It exports one function per request the application makes, fourteen in all, covering authentication and the current user, the catalogue and the prerequisite list, loading and saving the plan, the profile settings, the compliance check and the recommendation request. No other file in the client opens a network connection, which is what makes the layering above enforceable rather than merely intended: the client's whole dependence on the service is visible in one file, and the domain layer, which calls none of these functions, needs no service to run.

The module is also where the client and the service are reconciled. The service address is read from a build-time variable with a local fallback, so one build runs against a developer machine and against the deployed service without a code change. Every request carries the bearer token held in browser storage alongside the session cookie. The service's field names are converted to the client's, so the naming convention of the wire stops at this boundary. So does failure handling: a refused response becomes an exception carrying its status and the returned text, while two answers that could be read as failures are passed on as results instead, namely that nobody is signed in and that a programme publishes no prerequisites.

\subsection{What a Student Sees}
\label{sec:impl-frontend-components}

The layers above describe how the code is arranged. What they present is the three-panel shell of Section~\ref{sec:three-panels}: the catalogue, its filters and the Recommendation Panel on the left, the Graph View or the Table View in the centre, and the Dashboard on the right, reached through a gate that handles sign-in and, on first entry, collects the programme and start term (Section~\ref{sec:onboarding}). That gate also carries the study-condition control used during the evaluation. Each panel realises the corresponding section of Chapter~\ref{chap:design}, whose Figure~\ref{fig:three-panels} draws the arrangement, and the figures that follow show the built result rather than restating it: the shell as a student meets it (Figure~\ref{fig:impl-shot-shell}), then the Graph View (Figure~\ref{fig:impl-shot-graph}), the Table View (Figure~\ref{fig:impl-shot-table}), the two side panels (Figure~\ref{fig:impl-shot-recs}) and the catalogue sidebar beside the profile dialogue (Figure~\ref{fig:impl-shot-catalogue-profile}), each at a size where it can be read.
