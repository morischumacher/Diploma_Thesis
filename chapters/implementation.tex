...
The architecture strictly separates curriculum data from evaluation logic. Programme regulations are encoded in distinct JSON configurations loaded at construction, ensuring curriculum revisions require only data updates. Because the curricula structurally diverge (for example, the Bachelor requires an introductory \gterm{steop}{StEOP} phase, whereas the Master enforces prerequisite pairs), they share neither a JSON schema nor a common validation base class. Forcing a shared inheritance model would introduce premature architectural abstraction; instead, evaluation relies on programme-specific rule sets dynamically selected via programme code.
