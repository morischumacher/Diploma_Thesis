...
The client realises the interface designed in Chapter~\ref{chap:design} and is the larger of the two applications. It is organised in three layers, %% REV (not true if you look at figure, domain, not depending on api client) 
each depending only on the one below it, and what follows takes them in the order Figure~\ref{fig:impl-frontend-layers} draws them, from the top down.
...
That model is a reducer, a pure function from the current plan and a described change to the next plan. %% REV (you list these things already in the block above)
Marking a course complete, applying a prebuilt plan, switching programme, choosing a focus area and clearing the plan are dispatched to it as discrete changes, each exercisable by calling the function rather than by rendering the interface around it.
