...
\begin{itemize}
    ...
    \item \textbf{Recommendation data quality.} The content-based and sequence channels run on the catalogue's course content, its curated course similarities and the curricula's orderings, and on the student's own profile; the two behavioural channels run on a synthetic cohort (Section~\ref{sec:impl-recommender}). The consequence is that the evaluation observes how students treat an explainable suggestion, not how good the suggestions are, so the expertise-moderated pattern reported in Chapter~\ref{chap:discussion} is a finding about the workflow.
\end{itemize}
