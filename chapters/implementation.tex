...
\begin{itemize}
    \item \textbf{Single Plan, Two Projections:} Both views function as stateless rendering layers (Section~\ref{sec:impl-frontend}). The cost is that all UI-specific state must be structurally hoisted into the shared, persisted plan document, thereby coupling transient view logic to the global data model. However, this architectural trade-off guaranteed strict isolation of the independent variable during the user study.
