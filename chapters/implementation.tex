...
The client realises the interface designed in Chapter~\ref{chap:design} and is the larger of the two applications. It is organised in three layers, each depending only on the one below it, and what follows takes them in the order Figure~\ref{fig:impl-frontend-layers} draws them, from the top down.
...
The domain layer holds more than the plan and its reducer. The rules for terms and lanes, which decide the season a lane carries and whether a course may be offered in it; the lane geometry that turns a drop position into a semester; the graph's filter matching, which keeps a matched course's whole ancestry on screen; and the builders behind the prebuilt plans all live there on the same terms, each callable on its own without a browser around it.

\paragraph{The API Client}
Beneath both, a single module holds everything the client asks of the service, one exported function per request and fourteen in all: authentication and the current user, the catalogue and the prerequisite list, loading and saving the plan, the profile settings, the compliance check and the recommendation request. Nothing else in the client opens a connection, which is what makes the layering enforceable rather than merely intended, and this module is also where the service address, the bearer token and the wire's field names are settled, so that none of them appears anywhere above it.

\paragraph{What a Student Sees}
...
One qualification applies to this account, and it bounds the single authoritative model described above rather than contradicting it: the table's canvas holds each node's coordinates, and a course's horizontal coordinate encodes the semester it occupies, so that one derived value is reconciled in both directions rather than one.
