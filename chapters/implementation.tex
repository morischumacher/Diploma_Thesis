...
The layers above describe how the code is arranged. What they present is the three-panel shell of Section~\ref{sec:three-panels}, accessed through a gate that handles sign-in and collects the programme and start term on first entry (Section~\ref{sec:onboarding}). Figure~\ref{fig:three-panels} diagrams this overarching arrangement, while the subsequent figures capture the interface exactly as a student encounters it (Figure~\ref{fig:impl-shot-shell}). The layout centres on a primary workspace configured as either the Graph View (Figure~\ref{fig:impl-shot-graph}) or the Table View (Figure~\ref{fig:impl-shot-table}). This centrepiece is supported by the two side panels (Figure~\ref{fig:impl-shot-recs}), which house the Dashboard on the right, and the catalogue and Recommendation Panel on the left, shown alongside the profile dialogue in Figure~\ref{fig:impl-shot-catalogue-profile}.
...
The recommendation engine (implementing Feature~\ref{feat:007} and Feature~\ref{feat:012}) is designed strictly to evaluate the user experience of explainable recommendations, explicitly excluding algorithmic accuracy from the research scope (OOS5). The architecture employs a hybrid ensemble approach, combining content-based matching, rule-based sequence relations~\cite{wong_sequence_2018}, and collaborative filtering~\cite{bhumichitr_recommender_2017}. Because no record of prior students' plans was available, the collaborative components query a synthetic dataset of prior student plans. Consequently, the engine serves as a functional prototype that simulates data-driven UI behaviours for the evaluation study, rather than a contribution to recommender-systems research.

The system is structured as a federated pipeline of six scoring \gterm{recommendation-channel}{channels} (Interest, Similarity, Sequence, Follow-on, Peer, and Internship). Each adheres to a common interface, returning a numerical score and an explainable evidence string for a candidate course (Figure~\ref{fig:impl-shot-recs}). The channels employ distinct heuristics. For example, the Interest and Internship channels use weighted keyword matching, the Similarity channel surfaces curated catalogue links, and the Follow-on and Peer channels calculate probabilistic overlap against the synthetic cohort (Peer reverting to global popularity for an empty plan). After aggregating and deduplicating, the results are sorted by score, a relevance value in $[0,1]$ that each channel assigns on its own scale. The top 100 candidates are then passed to the Compliance Engine for trial placement, and any course that would introduce an error or a warning not already present in the plan is discarded. At most 15 recommendations are returned. %% REV [MAJOR] Contradicts the start of this same paragraph, which says the
%% Sequence and Follow-on channels could not fire, "so four channels were live
%% in the sessions Chapter 8 reports". This sentence says all six were active.
%% The Introduction's contribution bullet says four. Decide which count is
%% right; if it is four, a replacement that keeps the surviving information
%% without asserting a count is:
%%   Those channels that depend on the student's profile or on the cohort
%%   return nothing without input, so what a participant saw varied with what
%%   they had entered.
%% The number of live channels that are profile- or cohort-dependent was not
%% recomputed here, since that is a claim about the evaluated build.
All six channels were active during the sessions reported in Chapter~\ref{chap:evaluation}; the four that depend on the student's profile or on the cohort return nothing without input, so what a participant saw varied with what they had entered.
...
Munzner's nested model assigns specific threats to validity, and corresponding validation methods, to each of its four levels. The model warns of cascading errors: a failure at an inner level can invalidate outcomes at an outer level~\cite[p.~921]{munzner2009}. This chapter operates at the algorithm level. Verifying technical correctness here is essential to ensure that the subsequent user evaluation (Chapter~\ref{chap:evaluation}) measures genuine design efficacy rather than underlying software defects. Verification prioritises the Compliance Engine, because the evaluation findings rest on its accuracy and it is the surface participants read their plan's status from. The system is validated across three testing tiers:
...
One change was an addition rather than a correction. The overlay's scope was a decision, not an omission: Section~\ref{sec:graph-ltr} draws the hard prerequisites only, on the reasoning that an edge should carry a consequence. What the study exposed is a curriculum that defeats the reasoning. The orderings the Compliance Engine blocks on are two in the Master and none in the Bachelor, so a rule that draws only what blocks rendered nothing at all in the programme the study planned in. The same switch now draws the advisory orderings beside the enforced ones, dashed and labelled \emph{recommended before} where the enforced ones are solid and read \emph{required before}, so an edge states not only that an ordering exists but what breaking it costs: a refusal in the Master's case, a Dashboard warning in the Bachelor's. The pairs themselves are unchanged, two in each programme, and are the same copy the checkers read (Section~\ref{sec:impl-compliance}). No participant saw the advisory edges, so Chapter~\ref{chap:evaluation} carries no evidence about how they are read, and the code that asked for ordering at the point of placement is not answered by them: two edges in a view the student does not place in are not the indicator those four participants described (E-P44, 4/11).
