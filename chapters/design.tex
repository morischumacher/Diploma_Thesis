...
% ------------------------------------------------------------
\TDminor{Three perceptual sources are unavailable}{Ware 2004 (border-weight channel), Palmer 1992 (common region) and Shneiderman 1983 (direct manipulation) have no PDF in \texttt{context/}. Each is cited for a specific perceptual claim rather than in passing, so each needs opening once. Nielsen 1993, cited for the severity scale in Chapter~\ref{chap:evaluation}, is missing for the same reason.}
...
\TDmajor{Wienand et al.\ 2024 is cited but has never been opened}{This paragraph quotes the source directly (``index card-like presentation'') and Section~\ref{sec:dashboard} takes its three-level progress-bar pattern from it. There is no PDF anywhere in \texttt{context/}, and the bibliography entry describes an e-learning platform for enterprise-systems training, a different domain from study planning. A direct quotation from an unopened source in another domain is the most exposed citation in the thesis. Open access at \texttt{doi:10.1007/s44217-024-00165-z}; five minutes settles both uses.}
