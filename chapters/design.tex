...
This hierarchy is encoded as a fixed design constraint to match the institutional structure of the TU Wien programmes covered (Section~\ref{sec:overall-scope}; Figure~\ref{fig:curriculum-hierarchy}). A module containing exactly one Course Item is collapsed to that Course Item in the interface; an intermediate layer containing nothing but itself would add a hierarchy step without conveying information. OOS2 (Chapter~\ref{chap:methodology}) fixes the planning granularity at course-within-semester, which makes the Course Item the foundational level of this hierarchy.
