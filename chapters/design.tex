...
The chapter is therefore organised by design dimension rather than by screen region. It progresses through the general architecture and underlying data model, followed by visual encoding, interaction, and finally the individual views and features. This structure reflects Munzner's nested model, which treats visual encoding and interaction as a single, interdependent level of design~\cite[p.~922]{munzner2009}.
Core visual encodings, such as the obligation channel, the subject-colour hierarchy, and the course lifecycle, are properties of the overall system, not of an individual panel. Thus, the foundational dimensions are established first, allowing the later sections on specific views and features to focus strictly on what is unique to them without restating the underlying logic.

The design decisions presented below are guided by the nine design principles for a study planning assistant in higher education established by Bartel et al.~\cite{bartel_design_2024}, alongside the direct empirical evidence gathered from the formative interviews. In addition to these explicitly stated needs, several elements of the design originate directly from the Figma prototype used in those interviews (Chapter~\ref{chap:formative-study}). These elements were retained on the basis that six independent participant walkthroughs did not surface any objections to them. While this absence of objection provides indirect evidence that a concept is at least not actively unsuitable, it is acknowledged as weaker support than a formal user requirement. While these various inputs drive the design itself, the overarching scope of the tool follows directly from Chapter~\ref{chap:methodology} and Section~\ref{chap:from-themes-to-requirements}. Consequently, all five OOS boundaries (Section~\ref{sec:overall-scope}) serve as strictly settled constraints rather than decisions this chapter makes or reconsiders.

Ultimately, every interface decision presented below is explicitly justified by synthesising these various threads. The stated rationale for each choice is directly grounded in one of the foundations established above: a formal user requirement, a tolerated prototype assumption, common design knowledge, or specific published literature (such as the Bartel et al.~\cite{bartel_design_2024} principles or Nielsen's usability heuristics~\cite{nielsen1994}). Where a genuine alternative was weighed during design, it is recorded alongside its rejection reason; however, because not every decision had a live alternative worth naming, this is not claimed uniformly.
% ============================================================
...
This hierarchy is encoded as a fixed design constraint to match the institutional structure of the TU Wien programmes covered and is illustrated in Figure~\ref{fig:curriculum-hierarchy}. A module containing exactly one Course Item is collapsed to that Course Item in the interface; an intermediate layer containing nothing but itself would add a hierarchy step without conveying information. OOS2 (Chapter~\ref{chap:methodology}) fixes the planning granularity at course-within-semester, which makes the Course Item the foundational level of this hierarchy.
...
Feature~\ref{feat:002}, Req.~4 requires that the system distinguishes not-planned, planned, and completed states using consistent visual encodings across both views. Participants requested visual differentiation of states, \enquote{Differentiation… planned… completed}, and named the devices they expected: \enquote{A star means finished, for me}, \enquote{Maybe grey them out as well… you no longer need to highlight it visually} and \enquote{coloured in… which ones am I attending?} (T19). Feature~\ref{feat:011}, Req.~2 specifies that completed items should be faded while preserving an accessible record.
...
\begin{tikzpicture}[font=\scriptsize, scale=0.85, transform shape]
  ...
  \node[font=\scriptsize\bfseries] at (\ox+1.0,-0.4) {Done};
...
Figure~\ref{fig:colour-intensity} illustrates the final colour hierarchy.
Alternative encodings were considered but rejected. \emph{Full-saturation} subject colour on all cards was rejected because large coloured surfaces increase visual load, reduce text contrast, and flatten the intended hierarchy. \emph{Label-only subject encoding} was rejected because it forces serial reading instead of enabling immediate, pre-attentive grouping. \emph{Module-level hue assignment} was rejected because the high number of modules would exceed the set of reliably distinguishable hues, destroying cross-module coherence within a subject area.
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\small, scale=0.88, transform shape]
...
\begin{tikzpicture}[font=\scriptsize,
  ann/.style={font=\tiny\itshape, text=black, anchor=west},
  ...
  \node[ann] at (5.05,-2.28) {header};
  \draw[tick] (4.4,-3.3) -- (5.0,-3.3);
  \node[ann] at (5.05,-3.3) {title};
  \draw[tick] (4.4,-4.28) -- (5.0,-4.28);
  \node[ann] at (5.05,-4.28) {footer};
\end{tikzpicture}
...
Students build their curricula in an exploration-first sequence. As one participant noted, finding interesting courses is \enquote{cool as a first step… not thinking yet about which semester} (T20). Students distinctly separate the act of exploring options from the act of committing them to a timeline (T20). 
...
Students expect both explicit button actions for planning and direct spatial manipulation, often asking to \enquote{drag and drop somehow} (T21, T44). Consequently, the design treats these not as mutually exclusive choices, but as complementary interaction paths (Feature~\ref{feat:004}). Providing a visible button-based equivalent for every drag action aligns with Nielsen's recognition-rather-than-recall heuristic~\cite{nielsen1994}. Visible buttons make available actions salient immediately, rather than forcing users to guess if a drag gesture is supported. This dual approach keeps the action available to students unable to perform precise drag gestures (e.g., using a trackpad or with motor impairments) and supports workflows where dragging is impossible, such as adding a course directly from the Graph View. Furthermore, students require data-safe replanning capabilities (Feature~\ref{feat:004}, Req.~3; Feature~\ref{feat:008}, Req.~1, Req.~4). By leveraging the shared planning-state model (Section~\ref{sec:canvas}), moving a planned course acts as a direct state update. Because the Compliance Engine (Section~\ref{sec:compliance}) recomputes dynamically after every action, rapid replanning becomes a natural, immediate consequence of these interaction paths.
...
\begin{tikzpicture}[
  font=\sffamily,
  statebox/.style={draw=gray!60, fill=gray!5, rounded corners=4pt, line width=0.7pt,
                   minimum width=4.4cm, minimum height=1.25cm},
  btn/.style={draw=gray!50, fill=white, rounded corners=2pt, inner sep=2pt, font=\tiny},
  header/.style={font=\footnotesize\bfseries, text=black},
  tr/.style={-{Stealth[length=4.5pt]}, gray!70, line width=0.7pt},
  lbl/.style={font=\scriptsize, text=black, fill=white, inner sep=1pt}
]
  \def\xoff{7.0}
  \def\yoff{3.2}
  ...
  \node[header, anchor=north] at (NP.north) [yshift=-3pt] {Not Planned};
  \draw[gray!30, thin] ([yshift=\rule]NP.north west) -- ([yshift=\rule]NP.north east);
  \coordinate (npc) at ([yshift=\brow]NP.south);
  \node[btn, anchor=east] at ([xshift=-2pt]npc) {Park};
  \node[btn, anchor=west] at ([xshift=2pt]npc) {Add to plan};

  % 2. Parked
  \node[statebox] (PK) at (0, 0) {};
  \node[header, anchor=north] at (PK.north) [yshift=-3pt] {Parked};
  \draw[gray!30, thin] ([yshift=\rule]PK.north west) -- ([yshift=\rule]PK.north east);
  \coordinate (pkc) at ([yshift=\brow]PK.south);
  \node[btn, anchor=east] at ([xshift=-2pt]pkc) {Add to plan};
  \node[btn, anchor=west] at ([xshift=2pt]pkc) {Remove};

  % 3. Planned
  \node[statebox] (PL) at (\xoff, \yoff) {};
  \node[header, anchor=north] at (PL.north) [yshift=-3pt] {Planned};
  \draw[gray!30, thin] ([yshift=\rule]PL.north west) -- ([yshift=\rule]PL.north east);
  \node[btn] (plmid) at ([yshift=\brow]PL.south) {Mark done};
  \node[btn, anchor=east] at ([xshift=-3pt]plmid.west) {Move};  % opens the Semester Picker, Parking Stage included
  \node[btn, anchor=west] at ([xshift=3pt]plmid.east) {Remove};

  % 4. Done
  \node[statebox] (DO) at (\xoff, 0) {};
  \node[header, anchor=north] at (DO.north) [yshift=-3pt] {Done};
  \draw[gray!30, thin] ([yshift=\rule]DO.north west) -- ([yshift=\rule]DO.north east);
  \coordinate (doc) at ([yshift=\brow]DO.south);
  \node[btn, anchor=east] at ([xshift=-2pt]doc) {Unmark};
  \node[btn, anchor=west] at ([xshift=2pt]doc) {Remove};

  % Initial state marker
  \fill[black] (-3.0, \yoff) circle (0.09);
  \draw[tr, black] (-2.9, \yoff) -- (NP.west);
  ...
  % Parked <-> Planned (diagonal)
  \draw[tr] ([yshift=0.16cm]PK.north east) -- node[lbl, above, sloped, pos=0.20] {Add to plan} ([yshift=0.16cm]PL.south west);
  \draw[tr] ([yshift=-0.16cm]PL.south west) -- node[lbl, below, sloped, pos=0.80] {Park} ([yshift=-0.16cm]PK.north east);

  % Done -> NP (diagonal up-left)
  \draw[tr] (DO.north west) -- node[lbl, above, sloped, pos=0.11] {Remove} (NP.south east);
\end{tikzpicture}
\caption{Unified course lifecycle state machine with context-sensitive action buttons. Each state defines and displays only the active transitions available to the user.}
...
Based on this, a single compliance check is invoked after every planning action and reports the plan's status, its hard violations and its soft advisory warnings; what it computes, and how, is the subject of Chapter~\ref{chap:implementation} (Section~\ref{sec:impl-compliance}). Hard violations are surfaced as persistent, non-dismissible notifications; soft warnings as dismissible Dashboard notices; both update in real time, removing the compliance-checking loop from user memory. Judel et al.~\cite{judel_supporting_2023} describe an almost identical hard/soft distinction in a closely related study-planning tool, recommendations versus requirements, with each surfaced through distinct visual indicators rather than one undifferentiated warning; the Compliance Engine's separate treatments follow the same distinction.

Bartel et al.'s call for \emph{clearly recognisable} warnings within the same \enquote{\emph{support during planning difficulties}} guideline requires feedback to be immediately intelligible: if a student has to pause and deduce which of their recent actions triggered a violation, the burden the automatic check was meant to remove has been shifted rather than resolved. Compliance messages are therefore constructed to name the specific course, semester and action that triggered them. Generic static strings would force students to reconstruct their own interaction history, especially after several rapid adjustments; how the engine builds these messages is described in Section~\ref{sec:impl-compliance}.
...
\begin{enumerate}
  ...
  \item \textbf{Category level:} ECTS breakdowns by obligation category and by exam subject.
...
For workload management, the profile stores a recommended target and a maximum for both ECTS and weekly hours, and the two carry different force by design. The target is advice, so exceeding it is reported and the plan stands; the maximum is a limit, so a plan may not come to rest above it. Only credits are held to that limit: weekly hours are the student's own per-course estimate, and an estimate the student supplies is not one the tool should enforce against them, so those figures are reported rather than checked. This single set of thresholds also serves as the \enquote{constraint profile} during replanning (Feature~\ref{feat:008}, Req.~2). Following OOS1 (no personalised automated planning), the system reports, warns and refuses, but never re-balances a plan on the student's behalf. Judel et al.~\cite{judel_supporting_2023} report students in their own evaluation asking for the opposite, an automatic planning option computed from past performance and a credit ceiling, which marks this boundary as a scope decision rather than an omission.

The capacity profile is pre-populated with a default target of 30~ECTS per semester (the standard Bologna process full-time pace). This provides a safe, immediately useful baseline that students can override at any time. Conversely, the fields for academic interests and career direction, which two of the \gterm{recommendation-family}{recommendation families} draw on (Section~\ref{sec:rec-types-evidence}), start empty to avoid making baseless assumptions.

\emph{Fixed institution-wide limits} ignore individual tolerance (T4, T35, T48); \emph{automated load-balancing} violates OOS1; \emph{ECTS-only tracking} ignores the weekly hours participants also monitor (T49); \emph{self-assessed readiness} was scoped out to keep constraints verifiable; and \emph{empty defaults} demand too much setup before the tool is useful.

% ============================================================
\section{Recommendation Presentation Design}
\label{sec:recommendations}

Where the Dashboard reports what a plan contains (Section~\ref{sec:dashboard}), recommendations propose what it might contain next. Feature~\ref{feat:007} and Feature~\ref{feat:012} govern them, and they pull in two directions: a recommendation must carry enough inspectable evidence to be trusted, and it must reach the student without disturbing the canvas being worked on. The three subsections below take these in turn: what a recommendation is grounded in and how it names that grounding (Section~\ref{sec:rec-types-evidence}), where it is shown (Section~\ref{sec:rec-panel}), and what happens when a student accepts one (Section~\ref{sec:rec-accept}).
...
Rather than blending signals into an opaque, unified score, the system categorises recommendations into distinct \emph{families}. Each family evaluates a specific signal and names itself in words on the card it produces.
...
\begin{enumerate}
  \item \textbf{Interest match}: a course matching an interest the student named in the profile (Section~\ref{sec:profile}); the card names the matched interest.
  \item \textbf{Predefined similarity}: a course curated in the catalogue as similar to one the student has planned or completed; the card names that course and the curated reason.
  \item \textbf{Sequence or dependency}: a prerequisite or follow-up of a course already in the plan; the card names the relation. This realises Bartel et al.'s~\cite{bartel_design_2024} principle of clearly presenting curriculum information, extended here to the structural dependencies students cannot easily read from a raw handbook (T13).
\end{enumerate}

Three further families are conditional, evaluated only when the signal they need exists:
\begin{enumerate}
  \setcounter{enumi}{3}
  \item \textbf{Internship lens}: a course relevant to the career direction stated in the profile; the card names that direction.
...
The panel displays a scrollable list of recommendation cards, each draggable onto a semester lane (Section~\ref{sec:interaction-paths}) and each carrying the evidence the requirement asks for without displacing the course information a decision needs (Figure~\ref{fig:rec-panel}). A card names the family that produced it, the course by code, title and term availability, the human-readable reason (Section~\ref{sec:rec-types-evidence}), truncated with a show-more control so a long reason cannot push the card's other content out of view, the ECTS value and obligation type, and the same action buttons a course card carries elsewhere (Section~\ref{sec:action-buttons}). A toggle bar at the top of the panel shows or hides the families it presents, \emph{Interests}, \emph{Similarity}, \emph{Internships} and \emph{Other students}, and those preferences persist across sessions as part of the student's profile. Two families carry no switch of their own. \emph{Sequence or dependency} is shown whenever a relation exists, since what it reports follows from the curriculum rather than from a preference, and the same relations are separately drawn in the Graph View under a switch of their own; \emph{Follow-on from a completed course}, which reads the same cohort of prior-student plans as \emph{Other students}, is shown whenever that cohort supports it.
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\scriptsize,
  ...
  % panel
  \draw[gray!55, line width=0.9pt, rounded corners=4pt, fill=white] (0,0) rectangle (6.0,-7.6);
  ...
  \node[chip, anchor=west] (t3) at (0.3,-1.6) {Internships};
  \node[chip, anchor=west] (t4) at ([xshift=1.6mm]t3.east) {Other students};
  ...
  \draw[gray!60, line width=0.8pt, rounded corners=3pt, fill=white] (0.3,-7.2) rectangle (5.7,-5.4);
  ...
  \node[btn, anchor=east] (b2) at (5.55,-6.95) {add};
