...
%% REV (the courses should move to the left side where the other courses are, so a longer edge from exam subject B, so the edge is brighing the module level)
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
...
%% REV (the text in this figure is hard to read, please color it black, just for the done state it should be gray inside)
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\scriptsize, scale=0.85, transform shape]
...
%% (REV make sure to color the text in the figure black (the one on the right side))
\begin{figure}[htbp]
\centering
...
%% REV(make sure also this text on the left side of the figure is black)
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\scriptsize,
