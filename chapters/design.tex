...
\label{sec:visual}
This section details the visual encoding strategies used to represent the curriculum and user planning states. To satisfy the core requirement for a visual-first, low-text interface (Feature~\ref{feat:011}, Req.~1; C54), the design minimises textual clutter by leveraging visual channels.
...
This layout commits both axes, and that has a consequence worth stating plainly, because Research Question~1 names the understanding of course dependencies among the needs this work set out to serve. The horizontal axis encodes containment depth and the vertical axis encodes exam-subject membership, so a prerequisite relation, which runs between two courses and respects neither, cannot be given a position by the layout; it can only be overlaid on it. The graph as designed therefore draws two kinds of edge over one arrangement of nodes: containment edges from a parent to its child, which the axes place, and the curriculum's formal prerequisite pairs, which cross the exam-subject bands in a distinct edge style and are togglable so that they can be taken off the canvas when they are not wanted (Feature~\ref{feat:003}, Req.~2). What keeps that overlay cheap rather than cluttering is how little sequence the curricula in scope actually carry: the Bachelor programme encodes two prerequisite pairs across its 101 courses, \emph{Einführung in die Programmierung~1} before \emph{Einführung in die Programmierung~2} and \emph{Software Engineering} before \emph{Software Engineering Projekt}, and the Master programme encodes only the thesis before its defence and its seminar. What both curricula do encode is eligibility gates, the introductory phase in the Bachelor and the core-before-elective condition inside a Master focus area, and a gate is a condition to be satisfied rather than an edge to be drawn. Conditions are what the compliance engine evaluates and the Dashboard reports (Section~\ref{sec:compliance}), which is where they belong. This divides Feature~\ref{feat:003} across the two layers rather than assigning it wholly to one. Req.~2, the hard prerequisite relation, is carried in both: the engine blocks a placement that violates a pair, and the graph draws the pair. Req.~3, the soft or recommended relation, is carried in the checking layer alone, as a non-blocking Dashboard advisory, and the reason is a data one rather than a layout one: recommended sequence has no institutional grounding in either curriculum, so at design time there was no authoritative set of pairs to draw (Feature~\ref{feat:003}, Delineation; OOS3). A layout that gave sequence an axis of its own, rather than an overlay, would have had two edges to place and would have cost the subject-band encoding that Chapter~\ref{chap:evaluation} shows students did use; in a curriculum that genuinely encodes sequence, that trade would run the other way. Chapter~\ref{chap:evaluation} returns to what follows from this asymmetry for the students who asked to see recommended ordering at the point of decision and did not receive it, and Section~\ref{sec:impl-prereq-edges} describes the soft-dependency edge layer added to the tool after the evaluation study.
