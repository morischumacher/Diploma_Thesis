...
For workload management, the profile stores two thresholds for each of ECTS and weekly hours, a recommended target and a maximum, and the two are deliberately given different force. The recommended target is advice: exceeding it is reported in the Dashboard and the plan stands, which leaves a student free to carry a heavier semester knowingly (T4, T35). The maximum is a limit rather than advice, so a plan is not permitted to come to rest above it; a student who sets one is asking the tool to hold a line, and a line that produces one more warning is not one. That distinction is applied to credits, because credits are fixed by the curriculum, whereas the weekly hours are the student's own estimate entered per course, and an estimate the student supplies is not something the tool should then hold them to. The weekly figures are therefore carried to the Dashboard for the student's own reading rather than checked as a constraint. This single set of thresholds also serves as the \enquote{constraint profile} during replanning (Feature~\ref{feat:008}, Req.~2). Following OOS1 (no personalised automated planning), the system reports, warns and refuses, but never re-balances a plan on the student's behalf. Judel et al.~\cite{judel_supporting_2023} report students in their own evaluation asking for the opposite, an automatic planning option computed from past performance and a credit ceiling, which marks this boundary as a scope decision rather than an omission.
