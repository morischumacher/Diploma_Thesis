...
% ============================================================
\label{chap:design}
Following the process of obtaining features and requirements from the interview transcripts, this chapter documents the design of the study planning tool. It marks the shift from the previous chapter's focus on \emph{what} students need to this chapter's focus on \emph{how} the tool answers that need in interface terms; each section states a decision, the requirement or evidence it answers, and the alternatives it was weighed against. This chapter operates at level~3 of Munzner's nested model~\cite{munzner2009}, visual encoding and interaction design. Together with the Implementation chapter, it addresses Research Question~2, that is, how the requirements elicited in Chapter~\ref{chap:from-themes-to-requirements} can be realised as a hybrid graph-table interface.

The chapter is therefore organised by design dimension rather than by region of the screen: first the general architecture, then the visual encoding of curriculum elements, then interaction, and only then the individual views and features. This follows from the level Munzner's model places the chapter at. An encoding such as the obligation channel, the subject-colour hierarchy or the course lifecycle is a property of the design, not of a panel: the same encoding appears in the Graph View, in the Table View and in the catalogue, and it has to be derived once from the evidence that motivates it rather than three times from wherever it happens to be visible. Organising by screen region would split each of those decisions across the regions that display it. The view- and feature-specific sections that follow the three dimension sections therefore describe only what is genuinely particular to a view or a feature, and refer back to the encodings rather than restating them.

The design decisions presented below are guided by the 9 Design Principles for a Study Planning Assistant in Higher Education established by Bartel et al.~\cite{bartel_design_2024}, alongside the direct empirical evidence gathered from the formative interviews. A core recurring theme derived from these interviews is the necessity of a \emph{visual-first, low-text design} (Feature~\ref{feat:011}, Req.~1, grounded in C54's repeated preference for icon- and colour-based communication over text, e.g.\ \enquote{too much text is off-putting}). 
...
\end{tikzpicture}
\caption{Three-panel shell. The left panel's catalogue and filter controls follow the active view; the Recommendation Panel toggles independently of it. Centre: the active canvas. Right: the always-visible Dashboard.}
\label{fig:three-panels}
\end{figure}

\subsection{Unified Interactive Canvas for Both Views}
\label{sec:canvas}
Within that shell, the centre panel is where planning actually happens, and the next two subsections settle what it is made of. Spatial metaphors recurred when participants described that activity: ``\textit{like a desk… you just push things around}'' (C21), ``\textit{I'd rather place them how I want}'' (C28). A static, column-sorted table cannot express spatial priority, distance-based de-emphasis, or personal groupings that participants explicitly requested (C28, C29). Feature~\ref{feat:013}, Req.~1 (free grouping) and Feature~\ref{feat:013}, Req.~2 (positions persist) require the canvas to hold arbitrary item positions. Shneiderman's direct manipulation principle~\cite{shneiderman1983} supports a drag metaphor generally: the user moves items toward where they want them rather than navigating a form.
...
Both views compete for the same resource, horizontal space, and for different reasons: the Graph View needs width to show the curriculum tree at legible node sizes, the Table View to give every semester a column wide enough for its course cards. At standard screen widths there is not enough of it for both at a usable scale. Participants themselves differentiated the two views by purpose: ``\textit{graph used for exploration, not planning}'' (C20), which supports mutual exclusivity rather than permanent co-display.
...
Programme $\to$ Exam Subject $\to$ Module $\to$ Course Item. 

This hierarchy is encoded as a fixed design constraint to perfectly match the institutional structure of the TU Wien programmes covered (Section~\ref{sec:overall-scope}; Figure~\ref{fig:curriculum-hierarchy}). A module containing exactly one Course Item is collapsed to that Course Item in the interface; an intermediate layer containing nothing but itself would add a hierarchy step without conveying information. Furthermore, OOS2 (Chapter~\ref{chap:methodology}) confirms that no finer scheduling granularity than course-within-semester is required, validating that \enquote{Course Item} is the appropriate foundational level for this hierarchy.
...
\TDrev{Move the two single-course nodes into the Course column}{Your comment on Figure~\ref{fig:curriculum-hierarchy}: the two courses hanging off Exam Subject~B should sit in the Course column with the others, with a longer edge from Exam Subject~B crossing the Module level, so the figure shows the collapse rather than describing it.}
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  ...
  dots/.style={font=\scriptsize, text=gray!50}
]
  % 1st Level
  ...
  \node[cours] (c5)   at (7.0,-1.0) {Course};
  \node[cours] (c6)   at (7.0,-1.8) {Course};
  ...
  % Column Labels
  \node[font=\tiny, text=black!70] at (0,-2.95)    {Programme};
  \node[font=\tiny, text=black!70] at (3.5,-2.95)  {Exam Subject};
  \node[font=\tiny, text=black!70] at (7.0,-2.95)  {Module};
  \node[font=\tiny, text=black!70] at (10.5,-2.95) {Course};

  % The two single-course modules: the module level is not drawn, the course takes its place.
  \draw[{Stealth[length=3pt]}-, gray!55, thin] (8.15,-1.4) -- (9.3,-1.4);
  \node[font=\tiny, text=black!70, text width=2.9cm, align=center] at (10.6,-1.4)
    {no module level here: these modules hold one course};
\end{tikzpicture}
\caption{Four-level curriculum hierarchy. Modules holding several courses (top) keep their intermediate node. A module holding exactly one course (bottom) is not drawn at all: the course takes its place in the module column.}
\label{fig:curriculum-hierarchy}
\end{figure}

\section{Visual Design of Curriculum Elements}
\label{sec:visual}
\TDminor{Meta-discourse}{The opening clause describes the section rather than saying anything; start at ``To satisfy''.}
To satisfy the core requirement for a visual-first, low-text interface (Feature~\ref{feat:011}, Req.~1; C54), the design minimises textual clutter by leveraging visual channels.

% ------------------------------------------------------------
\TDminor{Three perceptual sources, and one bad file}{Dispositions under the unopened-source rule (\texttt{ANWEISUNGEN.md}~\S2). \emph{Shneiderman 1983} (direct manipulation, cited here and in Section~\ref{sec:interaction-paths}): the file committed to \texttt{context/related-work/} is a one-page browser print of the IEEE Xplore viewer, not the article, so it still needs downloading properly. \emph{Ware 2004} (border weight as a visual channel) and \emph{Palmer 1992} (common region): both are cited for the concept each is famous for, so if neither can be obtained, the sentences can stand as conventional attributions, or be reworded to claim the concept without attributing a specific finding. None of the three carries a quotation or a number, so none is blocking.}

\subsection{Course Obligation Encoding}
\label{sec:obligation}

Obligation type was named as a primary filter dimension: ``\textit{mandatory… narrow elective… broad elective}'' (C42). Obligation type must be readable on every card without opening any popover, because it is the most common criterion for deciding whether to explore a course further.

Based on Section~\ref{sec:course-obligation-abstraction}, a course or a module can have three obligation types: mandatory, constrained elective, fully elective (Figure~\ref{fig:obligation-rings}). This is encoded using the \emph{perceived border weight} channel~\cite{ware2004}: a thick border denotes mandatory; a medium border denotes constrained elective; a thin border denotes fully elective. A short text label naming the obligation category is additionally shown in the card footer, again as a secondary, redundant confirmation rather than the primary channel.

Border thickness as the primary channel is also a change from the prototype (Section~\ref{sec:prototype-elicitation}), not a carried-forward assumption: the prototype's obligation indicator was a visible letter badge (M/C/E) directly on each node, with border thickness introduced here instead and a text label introduced as the secondary confirmation just described. This shift is not traceable to a specific interview statement about obligation encoding itself, but it is a direct application of the visual-first, low-text principle (C54, Feature~\ref{feat:011}, Req.~1): a border-weight difference is read pre-attentively, while a letter requires serial reading.
...
\begin{tikzpicture}[font=\scriptsize, scale=0.85, transform shape]
  ...
  \node[font=\tiny,text=gray!55] at (1.2,-0.95) {thin border};
  ...
  \node[font=\tiny,text=gray!55] at (\ox+1.2,-0.95) {medium border};
  ...
  \node[font=\tiny,text=gray!55] at (\ox+1.2,-0.95) {thick border};
...
Feature~\ref{feat:002}, Req.~4 requires that the system distinguishes not-planned, planned, and completed states using consistent visual encodings across both views. Participants requested visual differentiation of states: ``\textit{greyed out}'' and ``\textit{star means finished}'' (C19), and ``\textit{colour-based marking for rapid recognition}'' (C19). Feature~\ref{feat:011}, Req.~2 specifies that completed items should be faded while preserving an accessible record.

To achieve this, each module and course card encodes its planning status using two pre-attentive visual channels: fill lightness and border saturation. Hue is explicitly avoided here because it is reserved for the subject-colour hierarchy (Section~\ref{sec:subject-colour}). Since lightness and saturation are perceptually separable from hue, both encoding systems can coexist without visual interference. The system distinguishes four distinct states:

\begin{itemize}
  \item Not planned: white fill, full-saturation subject-colour border.
  \item Parked: white fill, full-saturation subject-colour border, identical to not-planned; the parked state is distinguished by a text label in the card footer.
  \item Planned: light grey fill, full-saturation subject-colour border.
  \item Done: reduced opacity overall, border desaturated to neutral grey; the card ``recedes'' visually.
\end{itemize}
The states are also illustrated in Figure~\ref{fig:status-encoding}. To prevent confusion, a short text label (e.g., \enquote{planned}, \enquote{done}) in the card footer acts as a redundant confirmation of the visual state. It does not replace the visual encoding, but rather supports it. Because lightness and saturation are subtler than colour changes, relying on them alone could cause students to miss their planning status. Adding this small text label ensures clarity while still honouring the visual-first, low-text design required by Feature~\ref{feat:011}, Req.~1 (C54).
...
\TDrev{Figure~\ref{fig:status-encoding} text is too light}{Your comment: set the card text black, and keep grey only inside the Done state.}
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\scriptsize, scale=0.85, transform shape]
  ...
  \node[text=gray!25,font=\tiny,text width=1.8cm,align=center] at (1.0,1.8) {Course Name};
  \node[text=gray!25,font=\tiny] at (1.0,1.2) {4.5 ECTS};
  \node[font=\scriptsize\bfseries] at (1.0,-0.4) {Not Planned};
  \node[font=\tiny,text=gray!55,text width=2.1cm,align=center] at (1.0,-1.0) 
  {white fill, coloured border};
    ...
    \node[text=gray!25,font=\tiny,text width=1.8cm,align=center] at (\ox+1.0,1.55) {Course Name};
    \node[text=gray!25,font=\tiny] at (\ox+1.0,1.0) {4.5 ECTS};
    \node[font=\scriptsize\bfseries] at (\ox+1.0,-0.4) {Parked};
    \node[font=\tiny,text=gray!55,text width=2.1cm,align=center] at (\ox+1.0,-1.0)
    {white fill, label badge};
    % Card 3: Planned
    \def\ox{5.0}
    \fill[plannedfill, rounded corners=3pt] (\ox,0) rectangle (\ox+\cw,\ch);
    \draw[subjectblue, line width=2pt, rounded corners=3pt] (\ox,0) rectangle (\ox+\cw,\ch);
    \node[text=gray!45,font=\tiny,text width=1.8cm,align=center] at (\ox+1.0,1.8) {Course Name};
    \node[text=gray!45,font=\tiny] at (\ox+1.0,1.2) {4.5 ECTS};
  ...
  \node[font=\tiny,text=gray!55,text width=2.1cm,align=center] at (\ox+1.0,-1.0) {light grey fill, light grey border and text};
\end{tikzpicture}
...
The assigned hue is then propagated consistently across the visual hierarchy using three levels of intensity. This approach is grounded in the perceptual principle of \emph{common region}~\cite{palmer1992}, where elements located within the same visually defined area (even via shared tints or borders) are perceived as belonging together pre-attentively, reducing the need for text labels (Feature~\ref{feat:011}, Req.~1; C54).
\begin{enumerate}
...
\begin{tikzpicture}[font=\small, scale=0.88, transform shape]
  ...
  \draw[-{Stealth[length=5pt]}, gray!50, line width=0.8pt]
    (-0.8,0.8) -- (-0.8,5.4);
  \node[rotate=90, font=\scriptsize, text=gray!60] at (-1.2,3.1)
...
Overly dense text was reported as off-putting (C54). To support an always-current and highly legible planning view, students requested that key attributes like ECTS, obligation type, and completion status remain visible at a glance (C16). Furthermore, they explicitly requested that module completion status be visible directly on the canvas, rather than restricted solely to the Dashboard (C57; Feature~\ref{feat:002}, Req.~3). 

To achieve this, the design minimises text and adapts it to the informational role of each hierarchy level. Wienand et al.~\cite{wienand_design_2024}, designing an e-learning platform for a different domain, report that a complicated navigation structure was a source of student frustration in the system they replaced, and chunk their own content so that each unit is displayed as a tile, giving what they call an \enquote{index card-like presentation}. The parallel is with the chunking rather than with the domain: this interface adopts a similarly card-based layout to avoid dense text (C54), so the text within each unit is strictly constrained, and each level carries only what a decision at that level needs (Figure~\ref{fig:card-text}).

\TDok{Wienand et al.\ 2024, all three uses}{Opened and checked against the source, 2026-09-02. The quotation is exact: \enquote{The chapters of each course are displayed as tiles to get an index card-like presentation}, and the frustration finding it rests on (\enquote{a too complicated menu structure for WLIB's predecessor \dots\ led to a high level of frustration among the students}) is on the same page. Section~\ref{sec:dashboard}'s use holds too: the paper reports progress bars \enquote{in each chapter, each sub-chapter, and the whole course}, motivated by students who \enquote{might feel lost on how much progress they already made}, and its DP9 is literally \enquote{motivational elements}. The domain caveat already in this paragraph is what makes the transfer defensible; keep it.}

An exam-subject card is read for orientation, so it carries the subject name and the extent of what it contains, and nothing further. A module header is read to answer one question, whether the module's quota is met, so it aggregates: name, total ECTS, obligation type and a status indicator. A course card is where an actual placement decision is made, and it is divided into three fixed zones. Its header carries the short course-code abbreviation and the course-type indicator, applying Bartel et al.~\cite{bartel_design_2024} principle of relying on recognised university identifiers rather than inventing interface shorthand. Its title is the primary identifier and is clamped to two lines, which keeps card heights uniform for the snap-grid alignment system and keeps dense plans scannable. Its footer carries the three most frequently needed attributes: ECTS value, obligation type and status.

\TDrev{Figure~\ref{fig:card-text} annotation text}{Your comment: the annotations down the right-hand side should be black.}
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\scriptsize,
  ann/.style={font=\tiny, text=black!70, anchor=west, text width=4.6cm, align=left},
...
Importantly, the workflow remains flexible and does not force staging for every decision. A student who already knows their plan can bypass the Shortlist step entirely, assigning a course directly to a semester using the context-sensitive semester picker (Section~\ref{sec:semester-picker}), as illustrated by the direct assignment path in Figure~\ref{fig:workflow}.
...
\begin{tikzpicture}[font=\small, scale=0.88, transform shape]
  ...
  \node[font=\tiny, text=black!65, text width=2.9cm, align=center]
  ...
  \draw[-{Stealth[length=5pt]}, gray!70, dashed, line width=1pt]
    (\bw/2, -0.1) to[out=-20, in=-160] node[below, font=\scriptsize] {Direct Assign} (8.0+\bw/2, -0.1);

\end{tikzpicture}
\caption{Three-stage planning workflow: Browse (Graph View), Shortlist (Parking Stage), and Assign (Table View, committing to a semester). Students can also bypass the Shortlist stage to assign courses directly.}
\label{fig:workflow}
\end{figure}
...
\begin{tikzpicture}[
  ...
  lbl/.style={font=\scriptsize, text=gray!70, fill=white, inner sep=1pt}
]
  % States
...
\end{tikzpicture}
\caption{Course lifecycle state machine. Every course exists in exactly one of four states at any time.}
...
\begin{tikzpicture}[
  ...
  btn/.style={draw=gray!50, fill=white, rounded corners=2pt, inner sep=2.5pt, font=\scriptsize, text=gray!80},
  header/.style={font=\small\bfseries, text=black},
  tr/.style={-{Stealth[length=5pt]}, gray!80, line width=0.8pt},
  lbl/.style={font=\scriptsize, text=gray!70, fill=white, inner sep=2pt}
]
...
A recurring request was to personalise the planning space. Participants asked a tool where they could ``\textit{enter grades and hours and visualise}'' them (C12) while having ``\textit{space to write something next to it}'' (C47). To support this, the system must retain user-owned planning fields (specifically notes, grades, and workload estimates) across data refreshes (Feature~\ref{feat:001}, Req.~4) and allow inline note-taking per course (Feature~\ref{feat:004}, Req.~5).
...
Many students treat multi-course Master's modules as atomic units, preferring to add the entire module to their plan at once rather than course by course (C44, Feature~\ref{feat:004}). Students also requested clear visibility of their progress at the module level (C57, Feature~\ref{feat:002}). Furthermore, when interacting with these modules in the Graph View, it is critical that collapsing a module does not trigger a layout reflow; maintaining stable node positions is essential to preserve the user's spatial memory of the curriculum map (Feature~\ref{feat:013}, Req.~2).
...
% ------------------------------------------------------------
\TDminor{Figure caption lengths}{Against the house range of 100 to 200 characters: \texttt{fig:three-panels} is 207, \texttt{fig:curriculum-hierarchy} 215, \texttt{fig:workflow} 202 and \texttt{fig:state-machine} 94. All four are close; trim or extend on the next pass through this chapter.}

\section{Visual Legend Design}
\label{sec:legend}
Feature~\ref{feat:010} documents that the visual encodings are not self-evident on first exposure: participants asked ``\textit{what do these stand for?}'' (C41) and reported that shorthand labels are ``\textit{not intuitively understandable}'' (C40). Feature~\ref{feat:010}, Req.~1 requires an always-available legend explaining symbols, colours, and status encodings. This is a second, more targeted realisation of the same \enquote{clearly comprehensive content} principle (Bartel et al.~\cite{bartel_design_2024}) that the first-use tooltip sequence addresses more generally (Section~\ref{sec:onboarding}); the paper's specific recommendation of a \enquote{help bar (context-sensitive help)} describes almost exactly a togglable, on-demand legend rather than a one-off tutorial a student sees once and cannot return to.
...
\label{sec:graph-ltr}
To align with natural reading habits in Western contexts, participants suggested a horizontal layout, noting it is ``\textit{nicer if it goes left to right, because we read left to right}'' (Feature~\ref{feat:010}, C32). This direction maps increasing depth to increasing specificity while answering participants' questions about spatial meaning (``\textit{Is there any meaning to the vertical position?}'', C27).

Based on this, the curriculum tree renders left-to-right across four depth levels (Programme $\to$ Exam Subject $\to$ Module $\to$ Course; Figure~\ref{fig:graph-schema}). Exam subjects are stacked as distinct horizontal bands on the vertical axis, fulfilling Feature~\ref{feat:010}, Req.~2's \emph{structured layout mode} and directly answering C27: vertical band membership explicitly encodes the exam subject. Two alternatives were weighed against this. The Figma prototype's \emph{radial layout} (Section~\ref{sec:prototype-elicitation}) placed the programme at the centre with subjects radiating outward; it was re-derived and replaced because depth is then encoded as distance from a centre rather than as reading direction, which is what C32 asked for, and because a radial arrangement leaves no free axis on which subject membership could be encoded. A \emph{force-directed layout} was rejected because node positions would be recomputed whenever the graph is expanded or filtered, which would destroy the spatial memory Feature~\ref{feat:013}, Req.~2 requires the layout to preserve.
...
\begin{tikzpicture}[font=\scriptsize]
  ...
  % Column headers
  \node[font=\tiny, text=gray!60] at (3.3,7.5)  {Exam Subject};
  \node[font=\tiny, text=gray!60] at (6.0,7.5)  {Module};
  \node[font=\tiny, text=gray!60] at (9.2,7.5)  {Course};

  % Axis labels
  \draw[-{Stealth[length=4pt]}, gray!50] (2.2,-0.5) -- (11.2,-0.5);
  \node[font=\tiny, text=gray!60] at (6.7,-0.85)
    {$\longrightarrow$ Curriculum depth (left to right)};

  \draw[-{Stealth[length=4pt]}, gray!50] (-0.5,0) -- (-0.5,7.5);
  \node[rotate=90, font=\tiny, text=gray!60] at (-0.85,3.6)
...
A fully expanded 120-ECTS Master programme contains well over 100 nodes simultaneously, which overwhelms orientation before a student knows the layout (Feature~\ref{feat:009}, C50: ``\textit{with graphs it kind of explodes}''). Furthermore, students need to filter this content without losing context or deleting selections (Feature~\ref{feat:009}, Req.~1--3, C42).
...
To prevent information loss and context disruption, expanding a node automatically relaxes conflicting filters if it would otherwise reveal an empty result; and modifying filters never resets the graph's collapse state (Feature~\ref{feat:009}, Req.~2). Furthermore, filter configurations persist across sessions as stable planning preferences.
...
\label{sec:parking}
The Shortlist stage of the planning workflow (Section~\ref{sec:workflow}) needs a place on the canvas, described by participants as ``\textit{a pool with the courses where I can move them}'' (C21). It is realised as a Parking Stage positioned directly to the left of the active semester lanes, holding courses that have been explored and saved but not yet committed to a term. To visually separate it from active planning, the Parking Stage intentionally omits chronological and workload indicators, acting purely as a holding area. Because it shares the same continuous canvas as the semester lanes, students can directly evaluate parked courses alongside their committed schedule, seamlessly dragging items into a target semester when deciding  e.g. whether a course belongs in Semester 3 or Semester 4. If a student needs to focus exclusively on their active timeline without distractions, this staging area can be collapsed to reduce visual noise.
...
Manual compliance checking was described as effortful and unreliable: Feature~\ref{feat:003}, C13: ``\textit{Excel doesn't know your requirements… you have to double and triple check everything}''. Bartel et al.~\cite{bartel_design_2024} \enquote{\emph{support during planning difficulties}} principle calls specifically for design that contributes to avoiding planning errors and for \enquote{clearly recognisable warnings and hints in case of planning obstacles}, which real-time, automatic checking directly provides. Feature~\ref{feat:003}, Req.~1 (real-time fulfilment update), Req.~2 (hard rules blocking), Req.~3 (soft warnings non-blocking), Req.~4 (missing list updated in real time), and Req.~5 (compliance shared across views) together call for a single authoritative check invoked on every change, and institutional rules changing between academic years calls for that check to live somewhere updatable without an application release.
...
To satisfy the need to separate planned intent from actual completion (C19), the Dashboard features two top-level modes: \emph{Planning mode} (planned plus done) and \emph{Done mode} (done only). Crucially, the collapse states and section orders are tracked independently for each mode. A student can keep the Semester level expanded on top while looking ahead in Planning mode, but configure the Module level to sit on top when reviewing historical progress in Done mode. The two modes act as distinct, purpose-built views of progress rather than a single shared layout with a filter clumsily applied on top.

In addition to visualising current and historical data, the Dashboard functions as a real-time simulation tool that respects individual pacing preferences (C18). Aligning with Bartel et al.'s~\cite{bartel_design_2024} call for systems to account for planned semester workloads, the Semester-level progress bars dynamically evaluate the student's planned ECTS for each term against their recommended pacing target, which is read directly from their existing capacity profile (Section~\ref{sec:profile}). This ensures that students can instantly see if they are over- or under-loading a specific semester relative to their personal capacity without requiring redundant manual input.

Feature~\ref{feat:002}, Req.~6 mandates that a student must immediately observe the effect of adding or removing a course on their progress and completion projection. Because the Dashboard and Compliance Engine (Section~\ref{sec:compliance}) operate in real time, every planning action instantly recomputes ECTS totals, category breakdowns, and pacing projections. Consequently, a student can simulate \enquote{\textit{what happens if I add this course}} simply by dropping it onto the canvas, and undo the simulation just as easily by removing it. This leverages the established one-action interaction paths (Section~\ref{sec:interaction-paths}) without forcing the user into a separate, explicitly \enquote{tentative} preview mode.

Four alternatives were rejected. A \emph{single-level ECTS bar} hides category deficiencies behind a healthy total. A \emph{separate Dashboard screen} breaks the ambient, at-a-glance orientation C16 asks for. A \emph{shared section order across both modes} ignores that planning forward and reviewing back are different tasks. And a \emph{dedicated preview mode} would duplicate the shared state model (Section~\ref{sec:canvas}) when dropping a card onto the canvas already simulates the change.
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\scriptsize, scale=0.9, transform shape]
  ...
  \node[font=\tiny, text=gray!60, anchor=west] at (0.15,7.72) {67\,\% complete};
  ...
  \foreach \y in {8.3, 7.1, 5.6, 3.9} {
    \node[font=\tiny, text=gray!45] at (\pw-0.28,\y) {$\vee$};
  }

  % annotation
  \draw[-{Stealth[length=4pt]}, gray!50] (\pw+0.3,7.6) -- (\pw+0.9,7.6);
  \node[right, font=\tiny, text width=2.6cm] at (\pw+0.95,7.6) {Each level individually collapsible, order user-configurable};
\end{tikzpicture}
\caption{Progress Dashboard's four aggregation levels: Programme, Category, Semester, and Module. Each section collapses independently and the overall order is student-configurable.}
...
\emph{Fixed institution-wide limits} ignore individual tolerance (C4, C35, C48); \emph{automated load-balancing} violates OOS1; \emph{ECTS-only checking} ignores the weekly hours participants also monitor (C49); \emph{self-assessed readiness} was scoped out to keep constraints verifiable; and \emph{empty defaults} demand too much setup before the tool is useful.

% ============================================================
\section{Recommendation Presentation Design}
\label{sec:recommendations}
This section covers the design of how recommendations are \emph{displayed and interacted with}. The design of the underlying recommendation logic is treated separately; here the concern is how a recommendation, once computed, reaches the student without disrupting the planning context they have built.


% ------------------------------------------------------------

\TDminor{Meta-discourse at the section opening}{The two sentences opening Section~\ref{sec:recommendations} say what the section covers and what it does not. The headings carry both; cut them.}

\subsection{Recommendation Panel}
\label{sec:rec-panel}
Feature~\ref{feat:012}, Req.~2 requires that recommendations be accessible without resetting the canvas viewport, selection, or filters. Formative interviews motivated offloading this information into a separate panel (``\textit{otherwise it's too much on one page}'', C55) while explicitly warning against any feature that risks ``\textit{destroying the graph}'' (C43; Feature~\ref{feat:012}, Req.~1). Furthermore, Feature~\ref{feat:007}, Req.~2 and Req.~3 require explicit rationale and inspectable evidence per recommendation, which demands dedicated screen space. 

To meet these requirements without disrupting the user's spatial context, the Recommendation Panel is designed as an independent left-side drawer, positioned adjacent to the course catalogue (Section~\ref{sec:sidebar-design}). It can be toggled open or closed without affecting the canvas viewport, active filters, or node selections. Crucially, recommendations are strictly confined to this panel. No badges, tints, or rings are added to courses on the main canvas or in the catalogue. This strict isolation guarantees that recommendations cannot trigger a disruptive graph re-layout, and it avoids perceptual collisions with the dynamically assigned subject-colour palette (Section~\ref{sec:subject-colour}). 
...
\TDrev{Figure~\ref{fig:rec-panel} text}{Your comment: the text on the left side of the figure should be black too.}
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\scriptsize,
  ...
  btn/.style={draw=gray!50, fill=white, rounded corners=2pt, inner sep=1.6pt, font=\tiny, text=black!75},
  ann/.style={font=\tiny, text=black!70, anchor=west, text width=3.9cm, align=left},
  tick/.style={gray!55, thin}]
  ...
  \node[font=\tiny, text=black!70, text width=4.9cm, anchor=north west, align=left] at (0.45,-3.6)
  ...
  \node[font=\tiny, text=black!70, anchor=west] at (0.45,-6.55) {\emph{62\,\% of similar students chose this}};

  % annotations
  \draw[tick] (t3.east) -- (6.4,-1.05);
  \node[ann] at (6.45,-1.05) {family toggles, persisted};
  \draw[tick] (5.7,-2.2) -- (6.4,-2.2);
  \node[ann] at (6.45,-2.2) {the family that claimed the course};
  \draw[tick] (5.7,-3.75) -- (6.4,-3.75);
  \node[ann] at (6.45,-3.75) {the reason, truncated so a long one cannot displace the course details};
  \draw[tick] (b1.east) -- (6.4,-4.72);
  \node[ann] at (6.45,-4.72) {the same actions a course card carries};
\end{tikzpicture}
...
\begin{enumerate}
  \item \textbf{Interest match} ($\star$): Courses matching the student's stated profile interests (Section~\ref{sec:profile}). \emph{Evidence text:} Specifies the matched interest (e.g., ``matches your interests: focuses on Machine Learning'').
  \item \textbf{Predefined similarity} ($\approx$): Courses expert-curated in the metadata as similar to a course the student has already planned or completed. \emph{Evidence text:} Names the reference course and the curated rationale (e.g., ``similar to Web Engineering [shares a project-based assessment]'').
  \item \textbf{Sequence or dependency} ($\to$): Courses serving as a prerequisite or logical follow-up to courses already in the plan. \emph{Evidence text:} Names the dependency relation (e.g., ``prerequisite of Advanced Databases, which you have planned''). This realises Bartel et al.~\cite{bartel_design_2024} principle of clearly presenting curriculum information by surfacing structural dependencies students cannot easily read from a raw handbook.
\end{enumerate}

Three further families are conditional, evaluated only when the signal they need exists:
\begin{enumerate}
  \setcounter{enumi}{3}
  \item \textbf{Internship lens} ($\triangleright$): Courses relevant to the student's stated career direction. \emph{Evidence text:} Identifies the triggered career direction.
  \item \textbf{Follow-on from a completed course}: Courses that prior students commonly took after a course this student has already completed. \emph{Evidence text:} Names the completed course and the share (e.g., \enquote{62\,\% of students who completed Databases also took this}).
  \item \textbf{Peer / collaborative filtering}: Courses chosen by students whose plans resemble this one. To avoid overstating the data's confidence, the \emph{Evidence text} explicitly distinguishes between a genuine peer comparison (``62\,\% of students with a similar plan chose this'') and a cold-start overall popularity fallback when data is sparse (``popular choice: 40\,\% of prior students took this''). An honestly labelled weaker signal is inherently more trustworthy than a confident-sounding one.
\end{enumerate}

A \emph{single unified ranking} blending all six signals was rejected as the black box C23 objects to. \emph{Membership in several families at once} was rejected because merging rationale texts clutters the card; first-match-wins guarantees one transparent reason. \emph{Permanently expanded evidence} was rejected for the panel height it costs, and \emph{identical phrasing regardless of data quality} because it would overstate the algorithm's confidence.

% ------------------------------------------------------------
\subsection{Accept Flow}
\label{sec:rec-accept}

Accepting a recommendation seamlessly bridges the Recommendation Panel back into the core \emph{Browse $\to$ Shortlist $\to$ Assign} workflow (Section~\ref{sec:workflow}). To satisfy the requirement that accepting a recommendation must not disrupt the user's current context (Feature~\ref{feat:012}, Req.~3), the flow is executed entirely from the spatially independent panel (Section~\ref{sec:rec-panel}). Adding a course from this panel never alters the canvas viewport or node selection state. 
A recommended course is assigned through the same two paths as any other course (Feature~\ref{feat:007}, Req.~6; Section~\ref{sec:interaction-paths}): dragged from its card onto a semester lane, or added through the standard semester picker anchored to the card (Section~\ref{sec:semester-picker}). Once a semester is selected by either path, the course immediately transitions to a \emph{parked} or \emph{planned} state. The compliance engine re-evaluates the plan, updating the Dashboard. Crucially, because a recommendation for a course that is already in the plan is no longer actionable, the accepted card is immediately removed from the panel to keep the active recommendation list clean and focused.
