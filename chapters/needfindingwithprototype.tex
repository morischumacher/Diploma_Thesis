...
The analysis produced one artefact this thesis carries: the consolidated theme codebook, reproduced in Appendix~\ref{app:codebook} with each theme's excerpts and the number of interviews it recurred in. It is this chapter's substantive result and the input Chapter~\ref{chap:from-themes-to-requirements} works from: each theme can be read against the evidence that produced it, and each feature derived there names the themes it rests on, so the chain runs in both directions.