...
Each code was stored together with at least one verbatim quote and the corresponding interview ID. %% REV (we do did this yes, but we didnt add all the verbitms to the appendix, is this something you would add to the appendix? Lets discuss in the chat.)
This created an explicit analytic trail from interpretation back to source material and allowed later theme consolidation to remain grounded in the transcripts. Coding the six transcripts this way produced 435 initial codes, 81, 75, 56, 88, 69 and 66 across I1--I6.
...
Among the themes carried forward, seventeen recurred in four or more of the six interviews, and five recurred in every one: course choice weighed against feasibility (C6), the unification of the institutional and personal tools students otherwise consult separately (C10), automatic checking of curriculum rules instead of manual tracking (C13), recommendations that are grounded and personally relevant instead of generic (C23), and the representation of prerequisites and their order (C31). The remaining twelve add an overview spanning the current semester, the following ones and the whole programme with progress visible against it (C1, C16, C19); feasibility under personal circumstances that change (C4, C35); guidance where the fixed curriculum gives way to free choice (C5, C7); a usable institutional overview (C9); exploration and filtering of the catalogue before anything is committed to a semester (C20, C21, C42); and recommendations drawn from a student's interests, from content similarity and from dependencies between courses (C45).
